  ...
  physical_operator* op = new local_transform_op(
    ...
    new local_decrypt_op(
      ...
      new remote_sql_op(new param_generator_id19, "select agg_ident(custsale.cntrycode_DET), count(*) as numcust, hom_agg(:0, custsale.abal) from ( select " CUSTOMER_ENC_NAME ".c_phone_prefix_OPE as cntrycode, " CUSTOMER_ENC_NAME ".c_phone_prefix_DET as cntrycode_DET, " CUSTOMER_ENC_NAME ".c_packed_precompute_AGG as abal from " CUSTOMER_ENC_NAME " where ((" CUSTOMER_ENC_NAME ".c_phone_prefix_DET in ( :1, :2, :3, :4, :5, :6, :7 )) and ((" CUSTOMER_ENC_NAME ".c_acctbal_OPE) > (:_subselect$154))) and (not ( exists ( (select 1 from " ORDERS_ENC_NAME " where (" ORDERS_ENC_NAME ".o_custkey_DET) = (" CUSTOMER_ENC_NAME ".c_custkey_DET)) ) )) ) as custsale group by custsale.cntrycode order by custsale.cntrycode ASC", {db_column_desc(db_elem::TYPE_STRING, 2, oDET, SECLEVEL::DET, 4, false), db_column_desc(db_elem::TYPE_INT, 4, oNONE, SECLEVEL::PLAIN, 0, false), db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({std::pair<std::string, physical_operator*>("_subselect$154", new local_encrypt_op({std::pair<size_t, db_column_desc>(0, db_column_desc(db_elem::TYPE_DECIMAL_15_2, 8, oOPE, SECLEVEL::OPE, 5, false))}, new local_transform_op(
        ...
        new local_decrypt_op(
          {0},
          new remote_sql_op(new param_generator_id18, "select hom_agg(:0, " CUSTOMER_ENC_NAME ".c_packed_precompute_AGG), count(*) from " CUSTOMER_ENC_NAME " where ((" CUSTOMER_ENC_NAME ".c_acctbal_OPE) > (:1)) and (" CUSTOMER_ENC_NAME ".c_phone_prefix_DET in ( :2, :3, :4, :5, :6, :7, :8 ))", {db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false), db_column_desc(db_elem::TYPE_INT, 4, oNONE, SECLEVEL::PLAIN, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
      )
      ))})))
  )
  ;
