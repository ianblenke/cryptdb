  ...
  physical_operator* op = new local_limit(10,
    new local_order_by(
      ...
      new local_transform_op(
        {local_transform_op::trfm_desc(0), local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oNONE, SECLEVEL::PLAIN, 0, false), new function_call_node("hom_get_pos", {new tuple_pos_node(1), new int_literal_node(2)}))), local_transform_op::trfm_desc(2), local_transform_op::trfm_desc(3), },
        new local_decrypt_op(
          {0, 1, 2, 3},
          new remote_sql_op(new param_generator_id2, "select " LINEITEM_ENC_NAME ".l_orderkey_DET, hom_agg(:0, " LINEITEM_ENC_NAME ".l_packed_precompute_AGG), " ORDERS_ENC_NAME ".o_orderdate_DET, " ORDERS_ENC_NAME ".o_shippriority_DET from " CUSTOMER_ENC_NAME ", " ORDERS_ENC_NAME ", " LINEITEM_ENC_NAME " where (((((" CUSTOMER_ENC_NAME ".c_mktsegment_DET) = (:1)) and ((" CUSTOMER_ENC_NAME ".c_custkey_DET) = (" ORDERS_ENC_NAME ".o_custkey_DET))) and ((" LINEITEM_ENC_NAME ".l_orderkey_DET) = (" ORDERS_ENC_NAME ".o_orderkey_DET))) and ((" ORDERS_ENC_NAME ".o_orderdate_OPE) < (:2))) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) > (:3)) group by " LINEITEM_ENC_NAME ".l_orderkey_DET, " ORDERS_ENC_NAME ".o_orderdate_DET, " ORDERS_ENC_NAME ".o_shippriority_DET", {db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, false), db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG_ORIGINAL, SECLEVEL::SEMANTIC_AGG, 0, false), db_column_desc(db_elem::TYPE_DATE, 3, oDET, SECLEVEL::DET, 4, false), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DET, 7, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
      )
    )
  )
  ;
...
static void query_4(exec_context& ctx) {
  physical_operator* op = new local_order_by(
    ...
    new local_transform_op(
      {local_transform_op::trfm_desc(0), local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oNONE, SECLEVEL::PLAIN, 0, false), new function_call_node("hom_get_pos", {new tuple_pos_node(1), new int_literal_node(2)}))), },
      new local_decrypt_op(
        {0, 1},
        new remote_sql_op(new param_generator_id4, "select " NATION_ENC_NAME ".n_name_DET, hom_agg(:0, " LINEITEM_ENC_NAME ".l_packed_precompute_AGG) from " CUSTOMER_ENC_NAME ", " ORDERS_ENC_NAME ", " LINEITEM_ENC_NAME ", " SUPPLIER_ENC_NAME ", " NATION_ENC_NAME ", " REGION_ENC_NAME " where (((((((((" CUSTOMER_ENC_NAME ".c_custkey_DET) = (" ORDERS_ENC_NAME ".o_custkey_DET)) and ((" LINEITEM_ENC_NAME ".l_orderkey_DET) = (" ORDERS_ENC_NAME ".o_orderkey_DET))) and ((" LINEITEM_ENC_NAME ".l_suppkey_DET) = (" SUPPLIER_ENC_NAME ".s_suppkey_DET))) and ((" CUSTOMER_ENC_NAME ".c_nationkey_DET) = (" SUPPLIER_ENC_NAME ".s_nationkey_DET))) and ((" SUPPLIER_ENC_NAME ".s_nationkey_DET) = (" NATION_ENC_NAME ".n_nationkey_DET))) and ((" NATION_ENC_NAME ".n_regionkey_DET) = (" REGION_ENC_NAME ".r_regionkey_DET))) and ((" REGION_ENC_NAME ".r_name_DET) = (:1))) and ((" ORDERS_ENC_NAME ".o_orderdate_OPE) >= (:2))) and ((" ORDERS_ENC_NAME ".o_orderdate_OPE) < (:3)) group by " NATION_ENC_NAME ".n_name_DET", {db_column_desc(db_elem::TYPE_STRING, 25, oDET, SECLEVEL::DET, 1, false), db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG_ORIGINAL, SECLEVEL::SEMANTIC_AGG, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
    )
  )
  ;
  op->open(ctx);
...
static void query_5(exec_context& ctx) {
  physical_operator* op = new local_transform_op(
    {local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oNONE, SECLEVEL::PLAIN, 0, false), new function_call_node("hom_get_pos", {new tuple_pos_node(0), new int_literal_node(5)}))), },
    new local_decrypt_op(
      {0},
      new remote_sql_op(new param_generator_id5, "select hom_agg(:0, " LINEITEM_ENC_NAME ".l_packed_precompute_AGG) from " LINEITEM_ENC_NAME " where (((((" LINEITEM_ENC_NAME ".l_shipdate_OPE) >= (:1)) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) < (:2))) and ((" LINEITEM_ENC_NAME ".l_discount_OPE) >= (:3))) and ((" LINEITEM_ENC_NAME ".l_discount_OPE) <= (:4))) and ((" LINEITEM_ENC_NAME ".l_quantity_OPE) < (:5))", {db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG_ORIGINAL, SECLEVEL::SEMANTIC_AGG, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
  )
  ;
  op->open(ctx);
  while (op->has_more(ctx)) {
...
static void query_6(exec_context& ctx) {
  physical_operator* op = new local_transform_op(
    {local_transform_op::trfm_desc(0), local_transform_op::trfm_desc(1), local_transform_op::trfm_desc(2), local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oNONE, SECLEVEL::PLAIN, 0, false), new function_call_node("hom_get_pos", {new tuple_pos_node(3), new int_literal_node(2)}))), },
    new local_decrypt_op(
      {0, 1, 2, 3},
      new remote_sql_op(new param_generator_id6, "select agg_ident(shipping.supp_nation_DET), agg_ident(shipping.cust_nation_DET), agg_ident(shipping.l_year_DET), hom_agg(:0, shipping.volume) from ( select n1.n_name_OPE as supp_nation, n1.n_name_DET as supp_nation_DET, n2.n_name_OPE as cust_nation, n2.n_name_DET as cust_nation_DET, " LINEITEM_ENC_NAME ".l_shipdate_year_OPE as l_year, " LINEITEM_ENC_NAME ".l_shipdate_year_DET as l_year_DET, " LINEITEM_ENC_NAME ".l_packed_precompute_AGG as volume from " SUPPLIER_ENC_NAME ", " LINEITEM_ENC_NAME ", " ORDERS_ENC_NAME ", " CUSTOMER_ENC_NAME ", " NATION_ENC_NAME " n1, " NATION_ENC_NAME " n2 where ((((((((" SUPPLIER_ENC_NAME ".s_suppkey_DET) = (" LINEITEM_ENC_NAME ".l_suppkey_DET)) and ((" ORDERS_ENC_NAME ".o_orderkey_DET) = (" LINEITEM_ENC_NAME ".l_orderkey_DET))) and ((" CUSTOMER_ENC_NAME ".c_custkey_DET) = (" ORDERS_ENC_NAME ".o_custkey_DET))) and ((" SUPPLIER_ENC_NAME ".s_nationkey_DET) = (n1.n_nationkey_DET))) and ((" CUSTOMER_ENC_NAME ".c_nationkey_DET) = (n2.n_nationkey_DET))) and ((((n1.n_name_DET) = (:1)) and ((n2.n_name_DET) = (:2))) or (((n1.n_name_DET) = (:3)) and ((n2.n_name_DET) = (:4))))) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) >= (:5))) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) <= (:6)) ) as shipping group by shipping.supp_nation, shipping.cust_nation, shipping.l_year order by shipping.supp_nation ASC, shipping.cust_nation ASC, shipping.l_year ASC", {db_column_desc(db_elem::TYPE_STRING, 25, oDET, SECLEVEL::DET, 1, false), db_column_desc(db_elem::TYPE_STRING, 25, oDET, SECLEVEL::DET, 1, false), db_column_desc(db_elem::TYPE_INT, 2, oDET, SECLEVEL::DET, 0, false), db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG_ORIGINAL, SECLEVEL::SEMANTIC_AGG, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
  )
  ;
  op->open(ctx);
  while (op->has_more(ctx)) {
...
static void query_9(exec_context& ctx) {
  physical_operator* op = new local_limit(20,
    new local_order_by(
      ...
      new local_transform_op(
        {local_transform_op::trfm_desc(0), local_transform_op::trfm_desc(1), local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oNONE, SECLEVEL::PLAIN, 0, false), new function_call_node("hom_get_pos", {new tuple_pos_node(2), new int_literal_node(2)}))), local_transform_op::trfm_desc(3), local_transform_op::trfm_desc(4), local_transform_op::trfm_desc(5), local_transform_op::trfm_desc(6), local_transform_op::trfm_desc(7), },
        new local_decrypt_op(
          {0, 1, 2, 3, 4, 5, 6, 7},
          new remote_sql_op(new param_generator_id9, "select " CUSTOMER_ENC_NAME ".c_custkey_DET, " CUSTOMER_ENC_NAME ".c_name_DET, hom_agg(:0, " LINEITEM_ENC_NAME ".l_packed_precompute_AGG), " CUSTOMER_ENC_NAME ".c_acctbal_DET, " NATION_ENC_NAME ".n_name_DET, " CUSTOMER_ENC_NAME ".c_address_DET, " CUSTOMER_ENC_NAME ".c_phone_DET, " CUSTOMER_ENC_NAME ".c_comment_DET from " CUSTOMER_ENC_NAME ", " ORDERS_ENC_NAME ", " LINEITEM_ENC_NAME ", " NATION_ENC_NAME " where ((((((" CUSTOMER_ENC_NAME ".c_custkey_DET) = (" ORDERS_ENC_NAME ".o_custkey_DET)) and ((" LINEITEM_ENC_NAME ".l_orderkey_DET) = (" ORDERS_ENC_NAME ".o_orderkey_DET))) and ((" ORDERS_ENC_NAME ".o_orderdate_OPE) >= (:1))) and ((" ORDERS_ENC_NAME ".o_orderdate_OPE) < (:2))) and ((" LINEITEM_ENC_NAME ".l_returnflag_DET) = (:3))) and ((" CUSTOMER_ENC_NAME ".c_nationkey_DET) = (" NATION_ENC_NAME ".n_nationkey_DET)) group by " CUSTOMER_ENC_NAME ".c_custkey_DET, " CUSTOMER_ENC_NAME ".c_name_DET, " CUSTOMER_ENC_NAME ".c_acctbal_DET, " CUSTOMER_ENC_NAME ".c_phone_DET, " NATION_ENC_NAME ".n_name_DET, " CUSTOMER_ENC_NAME ".c_address_DET, " CUSTOMER_ENC_NAME ".c_comment_DET", {db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, false), db_column_desc(db_elem::TYPE_STRING, 25, oDET, SECLEVEL::DET, 1, false), db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG_ORIGINAL, SECLEVEL::SEMANTIC_AGG, 0, false), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 5, false), db_column_desc(db_elem::TYPE_STRING, 25, oDET, SECLEVEL::DET, 1, false), db_column_desc(db_elem::TYPE_STRING, 40, oDET, SECLEVEL::DET, 2, false), db_column_desc(db_elem::TYPE_STRING, 15, oDET, SECLEVEL::DET, 4, false), db_column_desc(db_elem::TYPE_STRING, 117, oDET, SECLEVEL::DET, 7, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
      )
    )
  )
  ;
    ...
    new local_transform_op(
      ...
      new local_decrypt_op(
        ...
        new local_group_filter(
          ...
          new local_decrypt_op(
            {0},
            new remote_sql_op(new param_generator_id10, "select hom_agg(:0, " PARTSUPP_ENC_NAME ".ps_packed_precompute_AGG), " PARTSUPP_ENC_NAME ".ps_partkey_DET from " PARTSUPP_ENC_NAME ", " SUPPLIER_ENC_NAME ", " NATION_ENC_NAME " where (((" PARTSUPP_ENC_NAME ".ps_suppkey_DET) = (" SUPPLIER_ENC_NAME ".s_suppkey_DET)) and ((" SUPPLIER_ENC_NAME ".s_nationkey_DET) = (" NATION_ENC_NAME ".n_nationkey_DET))) and ((" NATION_ENC_NAME ".n_name_DET) = (:1)) group by " PARTSUPP_ENC_NAME ".ps_partkey_DET", {db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG_ORIGINAL, SECLEVEL::SEMANTIC_AGG, 0, false), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
          , {
            new local_transform_op(
              ...
              new local_decrypt_op(
                {0},
                new remote_sql_op(new param_generator_id11, "select hom_agg(:0, " PARTSUPP_ENC_NAME ".ps_packed_precompute_AGG) from " PARTSUPP_ENC_NAME ", " SUPPLIER_ENC_NAME ", " NATION_ENC_NAME " where (((" PARTSUPP_ENC_NAME ".ps_suppkey_DET) = (" SUPPLIER_ENC_NAME ".s_suppkey_DET)) and ((" SUPPLIER_ENC_NAME ".s_nationkey_DET) = (" NATION_ENC_NAME ".n_nationkey_DET))) and ((" NATION_ENC_NAME ".n_name_DET) = (:1))", {db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG_ORIGINAL, SECLEVEL::SEMANTIC_AGG, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
            )
            ,
          }
        )
...
static void query_12(exec_context& ctx) {
  physical_operator* op = new local_transform_op(
    {local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DOUBLE, 8, oNONE, SECLEVEL::PLAIN, 0, false), new div_node(new mult_node(new double_literal_node(100.000000), new function_call_node("hom_get_pos", {new tuple_pos_node(1), new int_literal_node(2)})), new function_call_node("hom_get_pos", {new tuple_pos_node(0), new int_literal_node(2)})))), },
    new local_decrypt_op(
      {0, 1},
      new remote_sql_op(new param_generator_id13, "select hom_agg(:0, " LINEITEM_ENC_NAME ".l_packed_precompute_AGG), hom_agg(:1, case when searchSWP(:2, :3, " PART_ENC_NAME ".p_type_SWP) then " LINEITEM_ENC_NAME ".l_packed_precompute_AGG else NULL end) from " LINEITEM_ENC_NAME ", " PART_ENC_NAME " where (((" LINEITEM_ENC_NAME ".l_partkey_DET) = (" PART_ENC_NAME ".p_partkey_DET)) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) >= (:4))) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) < (:5))", {db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG_ORIGINAL, SECLEVEL::SEMANTIC_AGG, 0, false), db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG_ORIGINAL, SECLEVEL::SEMANTIC_AGG, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
  )
  ;
  op->open(ctx);
  while (op->has_more(ctx)) {
...
static void query_15(exec_context& ctx) {
  physical_operator* op = new local_transform_op(
    {local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oNONE, SECLEVEL::PLAIN, 0, false), new function_call_node("hom_get_pos", {new tuple_pos_node(0), new int_literal_node(2)}))), },
    new local_decrypt_op(
      {0},
      new remote_sql_op(new param_generator_id17, "select hom_agg(:0, " LINEITEM_ENC_NAME ".l_packed_precompute_AGG) from " LINEITEM_ENC_NAME ", " PART_ENC_NAME " where ((((((((((" PART_ENC_NAME ".p_partkey_DET) = (" LINEITEM_ENC_NAME ".l_partkey_DET)) and ((" PART_ENC_NAME ".p_brand_DET) = (:1))) and (" PART_ENC_NAME ".p_container_DET in ( :2, :3, :4, :5 ))) and ((" LINEITEM_ENC_NAME ".l_quantity_OPE) >= (:6))) and ((" LINEITEM_ENC_NAME ".l_quantity_OPE) <= (:7))) and (((" PART_ENC_NAME ".p_size_OPE) >= (:8)) and ((" PART_ENC_NAME ".p_size_OPE) <= (:9)))) and (" LINEITEM_ENC_NAME ".l_shipmode_DET in ( :10, :11 ))) and ((" LINEITEM_ENC_NAME ".l_shipinstruct_DET) = (:12))) or (((((((((" PART_ENC_NAME ".p_partkey_DET) = (" LINEITEM_ENC_NAME ".l_partkey_DET)) and ((" PART_ENC_NAME ".p_brand_DET) = (:13))) and (" PART_ENC_NAME ".p_container_DET in ( :14, :15, :16, :17 ))) and ((" LINEITEM_ENC_NAME ".l_quantity_OPE) >= (:18))) and ((" LINEITEM_ENC_NAME ".l_quantity_OPE) <= (:19))) and (((" PART_ENC_NAME ".p_size_OPE) >= (:20)) and ((" PART_ENC_NAME ".p_size_OPE) <= (:21)))) and (" LINEITEM_ENC_NAME ".l_shipmode_DET in ( :22, :23 ))) and ((" LINEITEM_ENC_NAME ".l_shipinstruct_DET) = (:24)))) or (((((((((" PART_ENC_NAME ".p_partkey_DET) = (" LINEITEM_ENC_NAME ".l_partkey_DET)) and ((" PART_ENC_NAME ".p_brand_DET) = (:25))) and (" PART_ENC_NAME ".p_container_DET in ( :26, :27, :28, :29 ))) and ((" LINEITEM_ENC_NAME ".l_quantity_OPE) >= (:30))) and ((" LINEITEM_ENC_NAME ".l_quantity_OPE) <= (:31))) and (((" PART_ENC_NAME ".p_size_OPE) >= (:32)) and ((" PART_ENC_NAME ".p_size_OPE) <= (:33)))) and (" LINEITEM_ENC_NAME ".l_shipmode_DET in ( :34, :35 ))) and ((" LINEITEM_ENC_NAME ".l_shipinstruct_DET) = (:36)))", {db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG_ORIGINAL, SECLEVEL::SEMANTIC_AGG, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
  )
  ;
  op->open(ctx);
  while (op->has_more(ctx)) {
...
static void query_16(exec_context& ctx) {
  physical_operator* op = new local_transform_op(
    ...
    new local_decrypt_op(
      {0, 2},
      new remote_sql_op(new param_generator_id19, "select agg_ident(custsale.cntrycode_DET), count(*) as numcust, hom_agg(:0, custsale.abal) from ( select " CUSTOMER_ENC_NAME ".c_phone_prefix_OPE as cntrycode, " CUSTOMER_ENC_NAME ".c_phone_prefix_DET as cntrycode_DET, " CUSTOMER_ENC_NAME ".c_packed_precompute_AGG as abal from " CUSTOMER_ENC_NAME " where ((" CUSTOMER_ENC_NAME ".c_phone_prefix_DET in ( :1, :2, :3, :4, :5, :6, :7 )) and ((" CUSTOMER_ENC_NAME ".c_acctbal_OPE) > (:_subselect$154))) and (not ( exists ( (select 1 from " ORDERS_ENC_NAME " where (" ORDERS_ENC_NAME ".o_custkey_DET) = (" CUSTOMER_ENC_NAME ".c_custkey_DET)) ) )) ) as custsale group by custsale.cntrycode order by custsale.cntrycode ASC", {db_column_desc(db_elem::TYPE_STRING, 2, oDET, SECLEVEL::DET, 4, false), db_column_desc(db_elem::TYPE_INT, 4, oNONE, SECLEVEL::PLAIN, 0, false), db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG_ORIGINAL, SECLEVEL::SEMANTIC_AGG, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({std::pair<std::string, physical_operator*>("_subselect$154", new local_encrypt_op({std::pair<size_t, db_column_desc>(0, db_column_desc(db_elem::TYPE_DECIMAL_15_2, 8, oOPE, SECLEVEL::OPE, 5, false))}, new local_transform_op(
        ...
        new local_decrypt_op(
          {0},
          new remote_sql_op(new param_generator_id18, "select hom_agg(:0, " CUSTOMER_ENC_NAME ".c_packed_precompute_AGG), count(*) from " CUSTOMER_ENC_NAME " where ((" CUSTOMER_ENC_NAME ".c_acctbal_OPE) > (:1)) and (" CUSTOMER_ENC_NAME ".c_phone_prefix_DET in ( :2, :3, :4, :5, :6, :7, :8 ))", {db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG_ORIGINAL, SECLEVEL::SEMANTIC_AGG, 0, false), db_column_desc(db_elem::TYPE_INT, 4, oNONE, SECLEVEL::PLAIN, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
      )
      ))})))
  )
  ;
