...
virtual param_map get_param_map(exec_context& ctx) {
  ...
  m[1] = db_elem((int64_t)encrypt_date_ope(ctx.crypto, 1020449 /*1993-1-1*/, _FP(10), _FJ(false)));
  m[2] = db_elem((int64_t)encrypt_date_ope(ctx.crypto, 1020961 /*1994-1-1*/, _FP(10), _FJ(false)));
  m[3] = db_elem(str_reverse(str_resize(encrypt_decimal_15_2_ope(ctx.crypto, 4 /*0.04*/, _FP(6), _FJ(false)), 16)));
  m[4] = db_elem(str_reverse(str_resize(encrypt_decimal_15_2_ope(ctx.crypto, 6 /*0.060000000000000005*/, _FP(6), _FJ(false)), 16)));
  m[5] = db_elem(str_reverse(str_resize(encrypt_decimal_15_2_ope(ctx.crypto, 2500 /*25*/, _FP(4), _FJ(false)), 16)));
  return m;
}
};
...
virtual param_map get_param_map(exec_context& ctx) {
  ...
  m[4] = db_elem((int64_t)encrypt_date_ope(ctx.crypto, 1022177 /*1996-7-1*/, _FP(10), _FJ(false)));
  m[5] = db_elem((int64_t)encrypt_date_ope(ctx.crypto, 1022209 /*1996-8-1*/, _FP(10), _FJ(false)));
  return m;
}
};
...
virtual param_map get_param_map(exec_context& ctx) {
  ...
  m[1] = db_elem(str_reverse(str_resize(encrypt_decimal_15_2_ope(ctx.crypto, 0 /*0.0*/, _FP(5), _FJ(false)), 16)));
  m[2] = db_elem(encrypt_string_det(ctx.crypto, "41", _FP(0), _FJ(false)));
  m[3] = db_elem(encrypt_string_det(ctx.crypto, "26", _FP(0), _FJ(false)));
  m[4] = db_elem(encrypt_string_det(ctx.crypto, "36", _FP(0), _FJ(false)));
  m[5] = db_elem(encrypt_string_det(ctx.crypto, "27", _FP(0), _FJ(false)));
  m[6] = db_elem(encrypt_string_det(ctx.crypto, "38", _FP(0), _FJ(false)));
  m[7] = db_elem(encrypt_string_det(ctx.crypto, "37", _FP(0), _FJ(false)));
  m[8] = db_elem(encrypt_string_det(ctx.crypto, "22", _FP(0), _FJ(false)));
  return m;
}
};
...
virtual param_map get_param_map(exec_context& ctx) {
  ...
  m[1] = db_elem(encrypt_string_det(ctx.crypto, "41", _FP(0), _FJ(false)));
  m[2] = db_elem(encrypt_string_det(ctx.crypto, "26", _FP(0), _FJ(false)));
  m[3] = db_elem(encrypt_string_det(ctx.crypto, "36", _FP(0), _FJ(false)));
  m[4] = db_elem(encrypt_string_det(ctx.crypto, "27", _FP(0), _FJ(false)));
  m[5] = db_elem(encrypt_string_det(ctx.crypto, "38", _FP(0), _FJ(false)));
  m[6] = db_elem(encrypt_string_det(ctx.crypto, "37", _FP(0), _FJ(false)));
  m[7] = db_elem(encrypt_string_det(ctx.crypto, "22", _FP(0), _FJ(false)));
  return m;
}
};
...
static void query_5(exec_context& ctx) {
  physical_operator* op = new local_transform_op(
    ...
    new local_decrypt_op(
      {0},
      new remote_sql_op(new param_generator_id5, "select agg_hash(:0, '/space/stephentu/data/tpch_1_00/lineitem_enc/row_pack/revenue', 256, 12, 't', lineitem_enc_cryptdb_opt_with_det.row_id) from lineitem_enc_cryptdb_opt_with_det where (((((lineitem_enc_cryptdb_opt_with_det.l_shipdate_OPE) >= (:1)) and ((lineitem_enc_cryptdb_opt_with_det.l_shipdate_OPE) < (:2))) and ((lineitem_enc_cryptdb_opt_with_det.l_discount_OPE) >= (:3))) and ((lineitem_enc_cryptdb_opt_with_det.l_discount_OPE) <= (:4))) and ((lineitem_enc_cryptdb_opt_with_det.l_quantity_OPE) < (:5))", {db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
  )
  ;
  op->open(ctx);
  while (op->has_more(ctx)) {
...
static void query_6(exec_context& ctx) {
  physical_operator* op = new local_order_by(
    ...
    new local_transform_op(
      ...
      new local_transform_op(
        {local_transform_op::trfm_desc(0), local_transform_op::trfm_desc(1), local_transform_op::trfm_desc(2), local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oNONE, SECLEVEL::PLAIN, 0, false), new mult_node(new tuple_pos_node(3), new sub_node(new int_literal_node(1), new tuple_pos_node(4))))), },
        new local_decrypt_op(
          {3, 4},
          new remote_sql_op(new param_generator_id6, "select n1.n_name_OPE as supp_nation, n2.n_name_OPE as cust_nation, lineitem_enc_cryptdb_opt_with_det.l_shipdate_year_OPE as l_year, array_to_string(array_agg(lineitem_enc_cryptdb_opt_with_det.l_extendedprice_DET), ','), array_to_string(array_agg(lineitem_enc_cryptdb_opt_with_det.l_discount_DET), ',') from supplier_enc_cryptdb_opt_with_det, lineitem_enc_cryptdb_opt_with_det, orders_enc_cryptdb_opt_with_det, customer_enc_cryptdb_opt_with_det, nation_enc_cryptdb_opt_with_det n1, nation_enc_cryptdb_opt_with_det n2 where ((((((((supplier_enc_cryptdb_opt_with_det.s_suppkey_DET) = (lineitem_enc_cryptdb_opt_with_det.l_suppkey_DET)) and ((orders_enc_cryptdb_opt_with_det.o_orderkey_DET) = (lineitem_enc_cryptdb_opt_with_det.l_orderkey_DET))) and ((customer_enc_cryptdb_opt_with_det.c_custkey_DET) = (orders_enc_cryptdb_opt_with_det.o_custkey_DET))) and ((supplier_enc_cryptdb_opt_with_det.s_nationkey_DET) = (n1.n_nationkey_DET))) and ((customer_enc_cryptdb_opt_with_det.c_nationkey_DET) = (n2.n_nationkey_DET))) and ((((n1.n_name_DET) = (:0)) and ((n2.n_name_DET) = (:1))) or (((n1.n_name_DET) = (:2)) and ((n2.n_name_DET) = (:3))))) and ((lineitem_enc_cryptdb_opt_with_det.l_shipdate_OPE) >= (:4))) and ((lineitem_enc_cryptdb_opt_with_det.l_shipdate_OPE) <= (:5)) group by n1.n_name_OPE, n2.n_name_OPE, lineitem_enc_cryptdb_opt_with_det.l_shipdate_year_OPE", {db_column_desc(db_elem::TYPE_STRING, 25, oOPE, SECLEVEL::OPE, 1, false), db_column_desc(db_elem::TYPE_STRING, 25, oOPE, SECLEVEL::OPE, 1, false), db_column_desc(db_elem::TYPE_INT, 2, oOPE, SECLEVEL::OPE, 0, false), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 5, true), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 6, true)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
      )
    )
  )
  ;
...
static void query_7(exec_context& ctx) {
  physical_operator* op = new local_order_by(
    ...
    new local_transform_op(
      ...
      new local_transform_op(
        {local_transform_op::trfm_desc(0), local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oNONE, SECLEVEL::PLAIN, 0, false), new mult_node(new tuple_pos_node(1), new sub_node(new int_literal_node(1), new tuple_pos_node(2))))), local_transform_op::trfm_desc(3), },
        new local_decrypt_op(
          {1, 2, 3},
          new remote_sql_op(new param_generator_id7, "select orders_enc_cryptdb_opt_with_det.o_orderdate_year_OPE as o_year, array_to_string(array_agg(lineitem_enc_cryptdb_opt_with_det.l_extendedprice_DET), ','), array_to_string(array_agg(lineitem_enc_cryptdb_opt_with_det.l_discount_DET), ','), array_to_string(array_agg(encode(n2.n_name_DET, 'hex')), ',') as nation from part_enc_cryptdb_opt_with_det, supplier_enc_cryptdb_opt_with_det, lineitem_enc_cryptdb_opt_with_det, orders_enc_cryptdb_opt_with_det, customer_enc_cryptdb_opt_with_det, nation_enc_cryptdb_opt_with_det n1, nation_enc_cryptdb_opt_with_det n2, region_enc_cryptdb_opt_with_det where (((((((((((part_enc_cryptdb_opt_with_det.p_partkey_DET) = (lineitem_enc_cryptdb_opt_with_det.l_partkey_DET)) and ((supplier_enc_cryptdb_opt_with_det.s_suppkey_DET) = (lineitem_enc_cryptdb_opt_with_det.l_suppkey_DET))) and ((lineitem_enc_cryptdb_opt_with_det.l_orderkey_DET) = (orders_enc_cryptdb_opt_with_det.o_orderkey_DET))) and ((orders_enc_cryptdb_opt_with_det.o_custkey_DET) = (customer_enc_cryptdb_opt_with_det.c_custkey_DET))) and ((customer_enc_cryptdb_opt_with_det.c_nationkey_DET) = (n1.n_nationkey_DET))) and ((n1.n_regionkey_DET) = (region_enc_cryptdb_opt_with_det.r_regionkey_DET))) and ((region_enc_cryptdb_opt_with_det.r_name_DET) = (:0))) and ((supplier_enc_cryptdb_opt_with_det.s_nationkey_DET) = (n2.n_nationkey_DET))) and ((orders_enc_cryptdb_opt_with_det.o_orderdate_OPE) >= (:1))) and ((orders_enc_cryptdb_opt_with_det.o_orderdate_OPE) <= (:2))) and ((part_enc_cryptdb_opt_with_det.p_type_DET) = (:3)) group by orders_enc_cryptdb_opt_with_det.o_orderdate_year_OPE", {db_column_desc(db_elem::TYPE_INT, 2, oOPE, SECLEVEL::OPE, 0, false), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 5, true), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 6, true), db_column_desc(db_elem::TYPE_STRING, 25, oDET, SECLEVEL::DET, 1, true)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
      )
    )
  )
  ;
  ...
  physical_operator* op = new local_order_by(
    ...
    new local_transform_op(
      ...
      new local_transform_op(
        ...
        new local_decrypt_op(
          {2, 3, 4},
          new remote_sql_op(new param_generator_id8, "select nation_enc_cryptdb_opt_with_det.n_name_OPE as nation, orders_enc_cryptdb_opt_with_det.o_orderdate_year_OPE as o_year, array_to_string(array_agg(lineitem_enc_cryptdb_opt_with_det.l_disc_price_DET), ','), array_to_string(array_agg(partsupp_enc_cryptdb_opt_with_det.ps_supplycost_DET), ','), array_to_string(array_agg(lineitem_enc_cryptdb_opt_with_det.l_quantity_DET), ',') from part_enc_cryptdb_opt_with_det, supplier_enc_cryptdb_opt_with_det, lineitem_enc_cryptdb_opt_with_det, partsupp_enc_cryptdb_opt_with_det, orders_enc_cryptdb_opt_with_det, nation_enc_cryptdb_opt_with_det where (((((((supplier_enc_cryptdb_opt_with_det.s_suppkey_DET) = (lineitem_enc_cryptdb_opt_with_det.l_suppkey_DET)) and ((partsupp_enc_cryptdb_opt_with_det.ps_suppkey_DET) = (lineitem_enc_cryptdb_opt_with_det.l_suppkey_DET))) and ((partsupp_enc_cryptdb_opt_with_det.ps_partkey_DET) = (lineitem_enc_cryptdb_opt_with_det.l_partkey_DET))) and ((part_enc_cryptdb_opt_with_det.p_partkey_DET) = (lineitem_enc_cryptdb_opt_with_det.l_partkey_DET))) and ((orders_enc_cryptdb_opt_with_det.o_orderkey_DET) = (lineitem_enc_cryptdb_opt_with_det.l_orderkey_DET))) and ((supplier_enc_cryptdb_opt_with_det.s_nationkey_DET) = (nation_enc_cryptdb_opt_with_det.n_nationkey_DET))) and (searchSWP(:0, :1, part_enc_cryptdb_opt_with_det.p_name_SWP)) group by nation_enc_cryptdb_opt_with_det.n_name_OPE, orders_enc_cryptdb_opt_with_det.o_orderdate_year_OPE", {db_column_desc(db_elem::TYPE_STRING, 25, oOPE, SECLEVEL::OPE, 1, false), db_column_desc(db_elem::TYPE_INT, 2, oOPE, SECLEVEL::OPE, 0, false), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 0, true), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 3, true), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 4, true)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
      )
    )
  )
  ;
...
static void query_10(exec_context& ctx) {
  physical_operator* op = new local_order_by(
    {std::make_pair(1, true)},
    new local_transform_op(
      {local_transform_op::trfm_desc(1), local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oNONE, SECLEVEL::PLAIN, 0, false), new sum_node(new tuple_pos_node(0), false))), },
      new local_decrypt_op(
        {1},
        new local_group_filter(
          new gt_node(new sum_node(new tuple_pos_node(0), false), new subselect_node(0, {})),
          new local_decrypt_op(
            {0},
            new remote_sql_op(new param_generator_id10, "select group_serializer(partsupp_enc_cryptdb_opt_with_det.virtual_global0), partsupp_enc_cryptdb_opt_with_det.ps_partkey_DET from partsupp_enc_cryptdb_opt_with_det, supplier_enc_cryptdb_opt_with_det, nation_enc_cryptdb_opt_with_det where (((partsupp_enc_cryptdb_opt_with_det.ps_suppkey_DET) = (supplier_enc_cryptdb_opt_with_det.s_suppkey_DET)) and ((supplier_enc_cryptdb_opt_with_det.s_nationkey_DET) = (nation_enc_cryptdb_opt_with_det.n_nationkey_DET))) and ((nation_enc_cryptdb_opt_with_det.n_name_DET) = (:0)) group by partsupp_enc_cryptdb_opt_with_det.ps_partkey_DET", {db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 0, true), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
          , {
            new local_transform_op(
              {local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DOUBLE, 8, oNONE, SECLEVEL::PLAIN, 0, false), new mult_node(new sum_node(new tuple_pos_node(0), false), new double_literal_node(0.000100)))), },
              new local_decrypt_op(
                {0},
                new remote_sql_op(new param_generator_id11, "select group_serializer(partsupp_enc_cryptdb_opt_with_det.virtual_global0) from partsupp_enc_cryptdb_opt_with_det, supplier_enc_cryptdb_opt_with_det, nation_enc_cryptdb_opt_with_det where (((partsupp_enc_cryptdb_opt_with_det.ps_suppkey_DET) = (supplier_enc_cryptdb_opt_with_det.s_suppkey_DET)) and ((supplier_enc_cryptdb_opt_with_det.s_nationkey_DET) = (nation_enc_cryptdb_opt_with_det.n_nationkey_DET))) and ((nation_enc_cryptdb_opt_with_det.n_name_DET) = (:0))", {db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 0, true)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
            )
            ,
          }
        )
...
static void query_11(exec_context& ctx) {
  physical_operator* op = new local_order_by(
    ...
    new local_decrypt_op(
      {0},
      new remote_sql_op(new param_generator_id12, "select lineitem_enc_cryptdb_opt_with_det.l_shipmode_DET, sum(case when ((orders_enc_cryptdb_opt_with_det.o_orderpriority_DET) = (:0)) or ((orders_enc_cryptdb_opt_with_det.o_orderpriority_DET) = (:1)) then 1 else 0 end) as high_line_count, sum(case when ((orders_enc_cryptdb_opt_with_det.o_orderpriority_DET) <> (:2)) and ((orders_enc_cryptdb_opt_with_det.o_orderpriority_DET) <> (:3)) then 1 else 0 end) as low_line_count from orders_enc_cryptdb_opt_with_det, lineitem_enc_cryptdb_opt_with_det where ((((((orders_enc_cryptdb_opt_with_det.o_orderkey_DET) = (lineitem_enc_cryptdb_opt_with_det.l_orderkey_DET)) and (lineitem_enc_cryptdb_opt_with_det.l_shipmode_DET in ( :4, :5 ))) and ((lineitem_enc_cryptdb_opt_with_det.l_commitdate_OPE) < (lineitem_enc_cryptdb_opt_with_det.l_receiptdate_OPE))) and ((lineitem_enc_cryptdb_opt_with_det.l_shipdate_OPE) < (lineitem_enc_cryptdb_opt_with_det.l_commitdate_OPE))) and ((lineitem_enc_cryptdb_opt_with_det.l_receiptdate_OPE) >= (:6))) and ((lineitem_enc_cryptdb_opt_with_det.l_receiptdate_OPE) < (:7)) group by lineitem_enc_cryptdb_opt_with_det.l_shipmode_DET", {db_column_desc(db_elem::TYPE_STRING, 10, oDET, SECLEVEL::DET, 14, false), db_column_desc(db_elem::TYPE_INT, 8, oNONE, SECLEVEL::PLAIN, 0, false), db_column_desc(db_elem::TYPE_INT, 8, oNONE, SECLEVEL::PLAIN, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
  )
  ;
  op->open(ctx);
  while (op->has_more(ctx)) {
...
static void query_12(exec_context& ctx) {
  physical_operator* op = new local_transform_op(
    {local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DOUBLE, 8, oNONE, SECLEVEL::PLAIN, 0, false), new div_node(new mult_node(new double_literal_node(100.000000), new function_call_node("hom_get_pos", {new tuple_pos_node(1), new int_literal_node(0)})), new function_call_node("hom_get_pos", {new tuple_pos_node(0), new int_literal_node(0)})))), },
    new local_decrypt_op(
      {0, 1},
      new remote_sql_op(new param_generator_id13, "select agg_hash(:0, '/space/stephentu/data/tpch_1_00/lineitem_enc/row_pack/disc_price', 256, 12, 't', lineitem_enc_cryptdb_opt_with_det.row_id), agg_hash(:1, '/space/stephentu/data/tpch_1_00/lineitem_enc/row_pack/disc_price', 256, 12, 't', case when searchSWP(:2, :3, part_enc_cryptdb_opt_with_det.p_type_SWP) then lineitem_enc_cryptdb_opt_with_det.row_id else null end) from lineitem_enc_cryptdb_opt_with_det, part_enc_cryptdb_opt_with_det where (((lineitem_enc_cryptdb_opt_with_det.l_partkey_DET) = (part_enc_cryptdb_opt_with_det.p_partkey_DET)) and ((lineitem_enc_cryptdb_opt_with_det.l_shipdate_OPE) >= (:4))) and ((lineitem_enc_cryptdb_opt_with_det.l_shipdate_OPE) < (:5))", {db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false), db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
  )
  ;
  op->open(ctx);
  while (op->has_more(ctx)) {
...
static void query_13(exec_context& ctx) {
  physical_operator* op = new local_transform_op(
    ...
    new local_decrypt_op(
      {0, 1},
      new remote_sql_op(new param_generator_id14, "select lineitem_enc_cryptdb_opt_with_det.l_partkey_DET, group_serializer(lineitem_enc_cryptdb_opt_with_det.l_quantity_DET) from lineitem_enc_cryptdb_opt_with_det, part_enc_cryptdb_opt_with_det where (((part_enc_cryptdb_opt_with_det.p_partkey_DET) = (lineitem_enc_cryptdb_opt_with_det.l_partkey_DET)) and ((part_enc_cryptdb_opt_with_det.p_brand_DET) = (:0))) and ((part_enc_cryptdb_opt_with_det.p_container_DET) = (:1)) group by lineitem_enc_cryptdb_opt_with_det.l_partkey_DET", {db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DET, 1, false), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 4, true)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
  )
  ;
  op->open(ctx);
  while (op->has_more(ctx)) {
...
static void query_14(exec_context& ctx) {
  physical_operator* op = new local_transform_op(
    ...
    new local_decrypt_op(
      {0, 1, 2, 3, 4, 5},
      new remote_sql_op(new param_generator_id16, "select customer_enc_cryptdb_opt_with_det.c_name_DET, customer_enc_cryptdb_opt_with_det.c_custkey_DET, orders_enc_cryptdb_opt_with_det.o_orderkey_DET, agg_ident(orders_enc_cryptdb_opt_with_det.o_orderdate_DET), agg_ident(orders_enc_cryptdb_opt_with_det.o_totalprice_DET), group_serializer(lineitem_enc_cryptdb_opt_with_det.l_quantity_DET) from customer_enc_cryptdb_opt_with_det, orders_enc_cryptdb_opt_with_det, lineitem_enc_cryptdb_opt_with_det where ((orders_enc_cryptdb_opt_with_det.o_orderkey_DET in ( :_subselect$0 )) and ((customer_enc_cryptdb_opt_with_det.c_custkey_DET) = (orders_enc_cryptdb_opt_with_det.o_custkey_DET))) and ((orders_enc_cryptdb_opt_with_det.o_orderkey_DET) = (lineitem_enc_cryptdb_opt_with_det.l_orderkey_DET)) group by customer_enc_cryptdb_opt_with_det.c_name_DET, customer_enc_cryptdb_opt_with_det.c_custkey_DET, orders_enc_cryptdb_opt_with_det.o_orderkey_DET, orders_enc_cryptdb_opt_with_det.o_orderdate_OPE, orders_enc_cryptdb_opt_with_det.o_totalprice_OPE order by orders_enc_cryptdb_opt_with_det.o_totalprice_OPE DESC, orders_enc_cryptdb_opt_with_det.o_orderdate_OPE ASC limit 100", {db_column_desc(db_elem::TYPE_STRING, 25, oDET, SECLEVEL::DET, 1, false), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, false), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, false), db_column_desc(db_elem::TYPE_DATE, 3, oDET, SECLEVEL::DET, 4, false), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 3, false), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 4, true)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({std::pair<std::string, physical_operator*>("_subselect$0", new local_transform_op(
        {local_transform_op::trfm_desc(1), },
        new local_group_filter(
          new gt_node(new sum_node(new tuple_pos_node(0), false), new int_literal_node(315)),
          new local_decrypt_op(
            {0},
            new remote_sql_op(new param_generator_id15, "select group_serializer(lineitem_enc_cryptdb_opt_with_det.l_quantity_DET), lineitem_enc_cryptdb_opt_with_det.l_orderkey_DET from lineitem_enc_cryptdb_opt_with_det group by lineitem_enc_cryptdb_opt_with_det.l_orderkey_DET", {db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 4, true), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
          , {
          }
        )
      )
      ...
      new remote_sql_op(new param_generator_id20, "select supplier_enc_cryptdb_opt_with_det.s_name_DET, supplier_enc_cryptdb_opt_with_det.s_address_DET from supplier_enc_cryptdb_opt_with_det, nation_enc_cryptdb_opt_with_det where ((supplier_enc_cryptdb_opt_with_det.s_suppkey_DET in ( :_subselect$1 )) and ((supplier_enc_cryptdb_opt_with_det.s_nationkey_DET) = (nation_enc_cryptdb_opt_with_det.n_nationkey_DET))) and ((nation_enc_cryptdb_opt_with_det.n_name_DET) = (:0))", {db_column_desc(db_elem::TYPE_STRING, 25, oDET, SECLEVEL::DET, 1, false), db_column_desc(db_elem::TYPE_STRING, 40, oDET, SECLEVEL::DET, 2, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({std::pair<std::string, physical_operator*>("_subselect$1", new local_encrypt_op({std::pair<size_t, db_column_desc>(0, db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 1, false))}, new local_transform_op(
        ...
        new local_filter_op(
          ...
          new local_transform_op(
            ...
            new local_decrypt_op(
              {0, 1, 2},
              new remote_sql_op(new param_generator_id19, "select partsupp_enc_cryptdb_opt_with_det.ps_suppkey_DET, group_serializer(partsupp_enc_cryptdb_opt_with_det.ps_availqty_DET), group_serializer(lineitem_enc_cryptdb_opt_with_det.l_quantity_DET) from partsupp_enc_cryptdb_opt_with_det, lineitem_enc_cryptdb_opt_with_det where (((((lineitem_enc_cryptdb_opt_with_det.l_partkey_DET) = (partsupp_enc_cryptdb_opt_with_det.ps_partkey_DET)) and ((lineitem_enc_cryptdb_opt_with_det.l_suppkey_DET) = (partsupp_enc_cryptdb_opt_with_det.ps_suppkey_DET))) and ((lineitem_enc_cryptdb_opt_with_det.l_shipdate_OPE) >= (:0))) and ((lineitem_enc_cryptdb_opt_with_det.l_shipdate_OPE) < (:1))) and (partsupp_enc_cryptdb_opt_with_det.ps_partkey_DET in ( :_subselect$0 )) group by partsupp_enc_cryptdb_opt_with_det.ps_partkey_DET, partsupp_enc_cryptdb_opt_with_det.ps_suppkey_DET", {db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 1, false), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DET, 2, true), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 4, true)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({std::pair<std::string, physical_operator*>("_subselect$0", new remote_sql_op(new param_generator_id18, "select part_enc_cryptdb_opt_with_det.p_partkey_DET from part_enc_cryptdb_opt_with_det where searchSWP(:0, :1, part_enc_cryptdb_opt_with_det.p_name_SWP)", {db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))})))
          )
          ,{
          }
        )
    ...
    new local_transform_op(
      ...
      new local_decrypt_op(
        ...
        new remote_sql_op(new param_generator_id23, "select custsale.cntrycode, count(*) as numcust, agg_hash(:0, '/space/stephentu/data/tpch_1_00/customer_enc/row_pack/acctbal', 256, 12, 't', custsale.abal) from ( select customer_enc_cryptdb_opt_with_det.c_phone_prefix_DET as cntrycode, customer_enc_cryptdb_opt_with_det.row_id as abal from customer_enc_cryptdb_opt_with_det where ((customer_enc_cryptdb_opt_with_det.c_phone_prefix_DET in ( :1, :2, :3, :4, :5, :6, :7 )) and ((customer_enc_cryptdb_opt_with_det.c_acctbal_OPE) > (:_subselect$0))) and (not ( exists ( (select 1 from orders_enc_cryptdb_opt_with_det where (orders_enc_cryptdb_opt_with_det.o_custkey_DET) = (customer_enc_cryptdb_opt_with_det.c_custkey_DET)) ) )) ) as custsale group by custsale.cntrycode", {db_column_desc(db_elem::TYPE_STRING, 15, oDET, SECLEVEL::DET, 4, false), db_column_desc(db_elem::TYPE_INT, 4, oNONE, SECLEVEL::PLAIN, 0, false), db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({std::pair<std::string, physical_operator*>("_subselect$0", new local_encrypt_op({std::pair<size_t, db_column_desc>(0, db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oOPE, SECLEVEL::OPE, 0, false))}, new local_transform_op(
          ...
          new local_decrypt_op(
            {0},
            new remote_sql_op(new param_generator_id22, "select agg_hash(:0, '/space/stephentu/data/tpch_1_00/customer_enc/row_pack/acctbal', 256, 12, 't', customer_enc_cryptdb_opt_with_det.row_id), count(*) from customer_enc_cryptdb_opt_with_det where ((customer_enc_cryptdb_opt_with_det.c_acctbal_OPE) > (:1)) and (customer_enc_cryptdb_opt_with_det.c_phone_prefix_DET in ( :2, :3, :4, :5, :6, :7, :8 ))", {db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false), db_column_desc(db_elem::TYPE_INT, 4, oNONE, SECLEVEL::PLAIN, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
        )
        ))})))
    )
  )
