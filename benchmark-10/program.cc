...
static void query_12(exec_context& ctx) {
  physical_operator* op = new local_transform_op(
    ...
    new local_decrypt_op(
      {0, 1},
      new remote_sql_op(new param_generator_id13, "select agg_hash(:0, '/space/stephentu/data/tpch_1_00/lineitem_enc/row_pack/disc_price', 256, 12, 't', case when searchSWP(:1, :2, part_enc_cryptdb_opt_with_det.p_type_SWP) then lineitem_enc_cryptdb_opt_with_det.row_id else null end), agg_hash(:3, '/space/stephentu/data/tpch_1_00/lineitem_enc/row_pack/disc_price', 256, 12, 't', lineitem_enc_cryptdb_opt_with_det.row_id) from lineitem_enc_cryptdb_opt_with_det, part_enc_cryptdb_opt_with_det where (((lineitem_enc_cryptdb_opt_with_det.l_partkey_DET) = (part_enc_cryptdb_opt_with_det.p_partkey_DET)) and ((lineitem_enc_cryptdb_opt_with_det.l_shipdate_OPE) >= (:4))) and ((lineitem_enc_cryptdb_opt_with_det.l_shipdate_OPE) < (:5))", {db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false), db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
  )
  ;
  op->open(ctx);
  while (op->has_more(ctx)) {
