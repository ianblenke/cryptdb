...
virtual param_map get_param_map(exec_context& ctx) {
  param_map m;
  m[0] = db_elem((int64_t)encrypt_date_ope(ctx.crypto, 1022497 /*1997-1-1*/, 10, true));
  m[1] = db_elem((int64_t)encrypt_date_ope(ctx.crypto, 1023009 /*1998-1-1*/, 10, true));
  {
    Binary key(ctx.crypto->cm->getKey(ctx.crypto->cm->getmkey(), fieldname(1, "SWP"), SECLEVEL::SWP));
...
static void query_12(exec_context& ctx) {
  physical_operator* op = new local_transform_op({local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DOUBLE, 8, oNONE, SECLEVEL::PLAIN, 0, false), new div_node(new mult_node(new double_literal_node(100.000000), new function_call_node("hom_get_pos", {new tuple_pos_node(1), new int_literal_node(0)})), new function_call_node("hom_get_pos", {new tuple_pos_node(0), new int_literal_node(0)})))), }, new local_decrypt_op({0, 1}, new remote_sql_op(new param_generator_id13, "select agg_hash(:0, '/space/stephentu/data/" DB_DATABASE "/lineitem_enc/row_pack/disc_price', 256, 12, false, " LINEITEM_ENC_NAME ".row_id), agg_hash(:1, '/space/stephentu/data/" DB_DATABASE "/lineitem_enc/row_pack/disc_price', 256, 12, false, case when searchSWP(:2, :3, " PART_ENC_NAME ".p_type_SWP) then " LINEITEM_ENC_NAME ".row_id else NULL end) from " LINEITEM_ENC_NAME ", " PART_ENC_NAME " where (((" LINEITEM_ENC_NAME ".l_partkey_DET) = (" PART_ENC_NAME ".p_partkey_DET)) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) >= (:4))) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) < (:5))", {db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false), db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({}))));
  op->open(ctx);
  while (op->has_more(ctx)) {
    physical_operator::db_tuple_vec v;
    op->next(ctx, v);
...
static void query_17(exec_context& ctx) {
  physical_operator* op = new local_order_by(
    ...
    new local_decrypt_op(
      {0, 1},
      new remote_sql_op(new param_generator_id22, "select " SUPPLIER_ENC_NAME ".s_name_DET, " SUPPLIER_ENC_NAME ".s_address_DET from " SUPPLIER_ENC_NAME ", " NATION_ENC_NAME " where ((" SUPPLIER_ENC_NAME ".s_suppkey_DET in ( :_subselect$1 )) and ((" SUPPLIER_ENC_NAME ".s_nationkey_DET) = (" NATION_ENC_NAME ".n_nationkey_DET))) and ((" NATION_ENC_NAME ".n_name_DET) = (:0))", {db_column_desc(db_elem::TYPE_STRING, 25, oDET, SECLEVEL::DET, 1, false), db_column_desc(db_elem::TYPE_STRING, 40, oDET, SECLEVEL::DET, 2, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({std::pair<std::string, physical_operator*>("_subselect$1",
        new local_transform_op(
          ...
          new local_filter_op(
            ...
            new local_transform_op(
              {local_transform_op::trfm_desc(0), local_transform_op::trfm_desc(1), local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oNONE, SECLEVEL::PLAIN, 0, false), new sum_node(new tuple_pos_node(2), false))), },
              new local_decrypt_op(
                {1, 2},
                new remote_sql_op(new param_generator_id21, "select " PARTSUPP_ENC_NAME ".ps_suppkey_DET, agg_ident(" PARTSUPP_ENC_NAME ".ps_availqty_DET), group_serializer(" LINEITEM_ENC_NAME ".l_quantity_DET) from " PARTSUPP_ENC_NAME ", " LINEITEM_ENC_NAME " where (((((" LINEITEM_ENC_NAME ".l_partkey_DET) = (" PARTSUPP_ENC_NAME ".ps_partkey_DET)) and ((" LINEITEM_ENC_NAME ".l_suppkey_DET) = (" PARTSUPP_ENC_NAME ".ps_suppkey_DET))) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) >= (:0))) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) < (:1))) and (" PARTSUPP_ENC_NAME ".ps_partkey_DET in ( select " PART_ENC_NAME ".p_partkey_DET from " PART_ENC_NAME " where searchSWP(:2, :3, " PART_ENC_NAME ".p_name_SWP) )) group by " PARTSUPP_ENC_NAME ".ps_partkey_DET, " PARTSUPP_ENC_NAME ".ps_suppkey_DET", {db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 1, false), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DET, 2, false), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 4, true)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
            )
            ,{
            }
          )
