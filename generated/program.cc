...
static void query_2(exec_context& ctx) {
  physical_operator* op = new local_limit(10, new local_order_by({std::make_pair(1, true), std::make_pair(2, false)}, new local_transform_op({local_transform_op::trfm_desc(1), local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oNONE, SECLEVEL::PLAIN, 0, false), new sum_node(new mult_node(new tuple_pos_node(2), new sub_node(new int_literal_node(1), new tuple_pos_node(3))), false))), local_transform_op::trfm_desc(4), local_transform_op::trfm_desc(5), }, new local_decrypt_op({1, 2, 3, 4, 5}, new local_filter_op(new lt_node(new tuple_pos_node(0), new date_literal_node("1995-3-12")), new local_decrypt_op({0}, new remote_sql_op(new param_generator_id2, "select array_to_string(array_agg(" ORDERS_ENC_NAME ".o_orderdate_DET), ','), " LINEITEM_ENC_NAME ".l_orderkey_DET, array_to_string(array_agg(" LINEITEM_ENC_NAME ".l_extendedprice_DET), ','), array_to_string(array_agg(" LINEITEM_ENC_NAME ".l_discount_DET), ','), " ORDERS_ENC_NAME ".o_orderdate_DET, " ORDERS_ENC_NAME ".o_shippriority_DET from " CUSTOMER_ENC_NAME ", " ORDERS_ENC_NAME ", " LINEITEM_ENC_NAME " where ((((" CUSTOMER_ENC_NAME ".c_mktsegment_DET) = (:0)) and ((" CUSTOMER_ENC_NAME ".c_custkey_DET) = (" ORDERS_ENC_NAME ".o_custkey_DET))) and ((" LINEITEM_ENC_NAME ".l_orderkey_DET) = (" ORDERS_ENC_NAME ".o_orderkey_DET))) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) > (:1)) group by " LINEITEM_ENC_NAME ".l_orderkey_DET, " ORDERS_ENC_NAME ".o_orderdate_DET, " ORDERS_ENC_NAME ".o_shippriority_DET", {db_column_desc(db_elem::TYPE_DATE, 3, oDET, SECLEVEL::DET, 4, true), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, false), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 5, true), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 6, true), db_column_desc(db_elem::TYPE_DATE, 3, oDET, SECLEVEL::DET, 4, false), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DET, 7, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({}))), {})))));
  op->open(ctx);
  while (op->has_more(ctx)) {
    physical_operator::db_tuple_vec v;
    op->next(ctx, v);
...
static void query_4(exec_context& ctx) {
  physical_operator* op = new local_order_by({std::make_pair(1, true)}, new local_transform_op({local_transform_op::trfm_desc(0), local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oNONE, SECLEVEL::PLAIN, 0, false), new sum_node(new mult_node(new tuple_pos_node(1), new sub_node(new int_literal_node(1), new tuple_pos_node(2))), false))), }, new local_decrypt_op({0, 1, 2}, new remote_sql_op(new param_generator_id4, "select " NATION_ENC_NAME ".n_name_DET, array_to_string(array_agg(" LINEITEM_ENC_NAME ".l_extendedprice_DET), ','), array_to_string(array_agg(" LINEITEM_ENC_NAME ".l_discount_DET), ',') from " CUSTOMER_ENC_NAME ", " ORDERS_ENC_NAME ", " LINEITEM_ENC_NAME ", " SUPPLIER_ENC_NAME ", " NATION_ENC_NAME ", " REGION_ENC_NAME " where (((((((((" CUSTOMER_ENC_NAME ".c_custkey_DET) = (" ORDERS_ENC_NAME ".o_custkey_DET)) and ((" LINEITEM_ENC_NAME ".l_orderkey_DET) = (" ORDERS_ENC_NAME ".o_orderkey_DET))) and ((" LINEITEM_ENC_NAME ".l_suppkey_DET) = (" SUPPLIER_ENC_NAME ".s_suppkey_DET))) and ((" CUSTOMER_ENC_NAME ".c_nationkey_DET) = (" SUPPLIER_ENC_NAME ".s_nationkey_DET))) and ((" SUPPLIER_ENC_NAME ".s_nationkey_DET) = (" NATION_ENC_NAME ".n_nationkey_DET))) and ((" NATION_ENC_NAME ".n_regionkey_DET) = (" REGION_ENC_NAME ".r_regionkey_DET))) and ((" REGION_ENC_NAME ".r_name_DET) = (:0))) and ((" ORDERS_ENC_NAME ".o_orderdate_OPE) >= (:1))) and ((" ORDERS_ENC_NAME ".o_orderdate_OPE) < (:2)) group by " NATION_ENC_NAME ".n_name_DET", {db_column_desc(db_elem::TYPE_STRING, 25, oDET, SECLEVEL::DET, 1, false), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 5, true), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 6, true)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))));
  op->open(ctx);
  while (op->has_more(ctx)) {
    physical_operator::db_tuple_vec v;
    op->next(ctx, v);
...
static void query_12(exec_context& ctx) {
  physical_operator* op = new local_transform_op({local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DOUBLE, 8, oNONE, SECLEVEL::PLAIN, 0, false), new div_node(new mult_node(new double_literal_node(100.000000), new sum_node(new case_when_node({new case_expr_case_node(new like_node(new tuple_pos_node(0), new string_literal_node("PROMO%"), false), new mult_node(new tuple_pos_node(1), new sub_node(new int_literal_node(1), new tuple_pos_node(2))))}, new int_literal_node(0)), false)), new sum_node(new mult_node(new tuple_pos_node(1), new sub_node(new int_literal_node(1), new tuple_pos_node(2))), false)))), }, new local_decrypt_op({0, 1, 2}, new remote_sql_op(new param_generator_id13, "select array_to_string(array_agg(encode(" PART_ENC_NAME ".p_type_DET, 'hex')), ','), array_to_string(array_agg(" LINEITEM_ENC_NAME ".l_extendedprice_DET), ','), array_to_string(array_agg(" LINEITEM_ENC_NAME ".l_discount_DET), ',') from " LINEITEM_ENC_NAME ", " PART_ENC_NAME " where (((" LINEITEM_ENC_NAME ".l_partkey_DET) = (" PART_ENC_NAME ".p_partkey_DET)) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) >= (:0))) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) < (:1))", {db_column_desc(db_elem::TYPE_STRING, 25, oDET, SECLEVEL::DET, 4, true), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 5, true), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 6, true)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({}))));
  op->open(ctx);
  while (op->has_more(ctx)) {
    physical_operator::db_tuple_vec v;
    op->next(ctx, v);
...
static void query_13(exec_context& ctx) {
  physical_operator* op = new local_transform_op({local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DOUBLE, 8, oNONE, SECLEVEL::PLAIN, 0, false), new div_node(new sum_node(new tuple_pos_node(2), false), new double_literal_node(7.000000)))), }, new local_decrypt_op({2}, new local_filter_op(new lt_node(new tuple_pos_node(0), new subselect_node(0, {new tuple_pos_node(1)})), new local_decrypt_op({0}, new remote_sql_op(new param_generator_id14, "select array_to_string(array_agg(" LINEITEM_ENC_NAME ".l_quantity_DET), ','), array_to_string(array_agg(" PART_ENC_NAME ".p_partkey_DET), ','), array_to_string(array_agg(" LINEITEM_ENC_NAME ".l_extendedprice_DET), ',') from " LINEITEM_ENC_NAME ", " PART_ENC_NAME " where (((" PART_ENC_NAME ".p_partkey_DET) = (" LINEITEM_ENC_NAME ".l_partkey_DET)) and ((" PART_ENC_NAME ".p_brand_DET) = (:0))) and ((" PART_ENC_NAME ".p_container_DET) = (:1))", {db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 4, true), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, true), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 5, true)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({}))), {new local_transform_op({local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DOUBLE, 8, oNONE, SECLEVEL::PLAIN, 0, false), new mult_node(new double_literal_node(0.200000), new avg_node(new tuple_pos_node(0), false)))), }, new local_decrypt_op({0}, new remote_sql_op(new param_generator_id15, "select array_to_string(array_agg(" LINEITEM_ENC_NAME ".l_quantity_DET), ',') from " LINEITEM_ENC_NAME " where (" LINEITEM_ENC_NAME ".l_partkey_DET) = (:0)", {db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 4, true)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))), })));
  op->open(ctx);
  while (op->has_more(ctx)) {
    physical_operator::db_tuple_vec v;
    op->next(ctx, v);
...
static void query_14(exec_context& ctx) {
  physical_operator* op = new local_transform_op({local_transform_op::trfm_desc(0), local_transform_op::trfm_desc(1), local_transform_op::trfm_desc(2), local_transform_op::trfm_desc(3), local_transform_op::trfm_desc(4), local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oNONE, SECLEVEL::PLAIN, 0, false), new sum_node(new tuple_pos_node(5), false))), }, new local_decrypt_op({0, 1, 2, 3, 4, 5}, new remote_sql_op(new param_generator_id17, "select " CUSTOMER_ENC_NAME ".c_name_DET, " CUSTOMER_ENC_NAME ".c_custkey_DET, " ORDERS_ENC_NAME ".o_orderkey_DET, " ORDERS_ENC_NAME ".o_orderdate_OPE, " ORDERS_ENC_NAME ".o_totalprice_OPE, array_to_string(array_agg(" LINEITEM_ENC_NAME ".l_quantity_DET), ',') from " CUSTOMER_ENC_NAME ", " ORDERS_ENC_NAME ", " LINEITEM_ENC_NAME " where ((" ORDERS_ENC_NAME ".o_orderkey_DET in ( :_subselect$1 )) and ((" CUSTOMER_ENC_NAME ".c_custkey_DET) = (" ORDERS_ENC_NAME ".o_custkey_DET))) and ((" ORDERS_ENC_NAME ".o_orderkey_DET) = (" LINEITEM_ENC_NAME ".l_orderkey_DET)) group by " CUSTOMER_ENC_NAME ".c_name_DET, " CUSTOMER_ENC_NAME ".c_custkey_DET, " ORDERS_ENC_NAME ".o_orderkey_DET, " ORDERS_ENC_NAME ".o_orderdate_OPE, " ORDERS_ENC_NAME ".o_totalprice_OPE order by " ORDERS_ENC_NAME ".o_totalprice_OPE DESC, " ORDERS_ENC_NAME ".o_orderdate_OPE ASC limit 100", {db_column_desc(db_elem::TYPE_STRING, 25, oDET, SECLEVEL::DET, 1, false), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, false), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, false), db_column_desc(db_elem::TYPE_DATE, 3, oOPE, SECLEVEL::OPE, 4, false), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oOPE, SECLEVEL::OPE, 3, false), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 4, true)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({std::pair<std::string, physical_operator*>("_subselect$1", new local_transform_op({local_transform_op::trfm_desc(1), }, new local_group_filter(new gt_node(new sum_node(new tuple_pos_node(0), false), new int_literal_node(315)), new local_decrypt_op({0}, new remote_sql_op(new param_generator_id16, "select array_to_string(array_agg(" LINEITEM_ENC_NAME ".l_quantity_DET), ','), " LINEITEM_ENC_NAME ".l_orderkey_DET from " LINEITEM_ENC_NAME " group by " LINEITEM_ENC_NAME ".l_orderkey_DET", {db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 4, true), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({}))), {})))}))));
  op->open(ctx);
  while (op->has_more(ctx)) {
    physical_operator::db_tuple_vec v;
    op->next(ctx, v);
