...
static void query_3(exec_context& ctx) {
  physical_operator* op = new local_order_by({std::make_pair(0, false)}, new local_decrypt_op({0}, new remote_sql_op(new param_generator_id3, "select orders_enc.o_orderpriority_DET, count(*) as order_count from orders_enc where (((orders_enc.o_orderdate_OPE) >= (:0)) and ((orders_enc.o_orderdate_OPE) < (:1))) and (exists ( (select 1 from lineitem_enc where ((lineitem_enc.l_orderkey_DET) = (orders_enc.o_orderkey_DET)) and ((lineitem_enc.l_commitdate_OPE) < (lineitem_enc.l_receiptdate_OPE))) )) group by orders_enc.o_orderpriority_DET", {db_column_desc(db_elem::TYPE_STRING, 15, oDET, SECLEVEL::DET, 5, false), db_column_desc(db_elem::TYPE_INT, 4, oNONE, SECLEVEL::PLAIN, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({}))));
  op->open(ctx);
  while (op->has_more(ctx)) {
    physical_operator::db_tuple_vec v;
    op->next(ctx, v);
...
static void query_13(exec_context& ctx) {
  physical_operator* op = new local_transform_op({local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DOUBLE, 8, oNONE, SECLEVEL::PLAIN, 0, false), new div_node(new sum_node(new tuple_pos_node(2), false), new double_literal_node(7.000000)))), }, new local_decrypt_op({2}, new local_filter_op(new lt_node(new tuple_pos_node(0), new subselect_node(0, {new tuple_pos_node(1)})), new local_decrypt_op({0}, new remote_sql_op(new param_generator_id14, "select array_to_string(array_agg(lineitem_enc.l_quantity_DET), ','), array_to_string(array_agg(part_enc.p_partkey_DET), ','), array_to_string(array_agg(lineitem_enc.l_extendedprice_DET), ',') from lineitem_enc, part_enc where (((part_enc.p_partkey_DET) = (lineitem_enc.l_partkey_DET)) and ((part_enc.p_brand_DET) = (:0))) and ((part_enc.p_container_DET) = (:1))", {db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 4, true), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, true), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 5, true)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({}))), {new local_transform_op({local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DOUBLE, 8, oNONE, SECLEVEL::PLAIN, 0, false), new mult_node(new double_literal_node(0.200000), new avg_node(new tuple_pos_node(0), false)))), }, new local_decrypt_op({0}, new remote_sql_op(new param_generator_id15, "select array_to_string(array_agg(lineitem_enc.l_quantity_DET), ',') from lineitem_enc where (lineitem_enc.l_partkey_DET) = (:0)", {db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 4, true)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))), })));
  op->open(ctx);
  while (op->has_more(ctx)) {
    physical_operator::db_tuple_vec v;
    op->next(ctx, v);
...
static void query_14(exec_context& ctx) {
  physical_operator* op = new local_transform_op({local_transform_op::trfm_desc(0), local_transform_op::trfm_desc(1), local_transform_op::trfm_desc(2), local_transform_op::trfm_desc(3), local_transform_op::trfm_desc(4), local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oNONE, SECLEVEL::PLAIN, 0, false), new sum_node(new tuple_pos_node(5), false))), }, new local_decrypt_op({0, 1, 2, 3, 4, 5}, new remote_sql_op(new param_generator_id17, "select customer_enc.c_name_DET, customer_enc.c_custkey_DET, orders_enc.o_orderkey_DET, orders_enc.o_orderdate_OPE, orders_enc.o_totalprice_OPE, array_to_string(array_agg(lineitem_enc.l_quantity_DET), ',') from customer_enc, orders_enc, lineitem_enc where ((orders_enc.o_orderkey_DET in ( :_subselect$1 )) and ((customer_enc.c_custkey_DET) = (orders_enc.o_custkey_DET))) and ((orders_enc.o_orderkey_DET) = (lineitem_enc.l_orderkey_DET)) group by customer_enc.c_name_DET, customer_enc.c_custkey_DET, orders_enc.o_orderkey_DET, orders_enc.o_orderdate_OPE, orders_enc.o_totalprice_OPE order by orders_enc.o_totalprice_OPE DESC, orders_enc.o_orderdate_OPE ASC limit 100", {db_column_desc(db_elem::TYPE_STRING, 25, oDET, SECLEVEL::DET, 1, false), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, false), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, false), db_column_desc(db_elem::TYPE_DATE, 3, oOPE, SECLEVEL::OPE, 4, false), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oOPE, SECLEVEL::OPE, 3, false), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 4, true)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({std::pair<std::string, physical_operator*>("_subselect$1", new local_transform_op({local_transform_op::trfm_desc(1), }, new local_group_filter(new gt_node(new sum_node(new tuple_pos_node(0), false), new int_literal_node(315)), new local_decrypt_op({0}, new remote_sql_op(new param_generator_id16, "select array_to_string(array_agg(lineitem_enc.l_quantity_DET), ','), lineitem_enc.l_orderkey_DET from lineitem_enc group by lineitem_enc.l_orderkey_DET", {db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 4, true), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({}))), {})))}))));
  op->open(ctx);
  while (op->has_more(ctx)) {
    physical_operator::db_tuple_vec v;
    op->next(ctx, v);
...
static void query_15(exec_context& ctx) {
  physical_operator* op = new local_transform_op({local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oNONE, SECLEVEL::PLAIN, 0, false), new sum_node(new mult_node(new tuple_pos_node(0), new sub_node(new int_literal_node(1), new tuple_pos_node(1))), false))), }, new local_decrypt_op({0, 1}, new remote_sql_op(new param_generator_id18, "select array_to_string(array_agg(lineitem_enc.l_extendedprice_DET), ','), array_to_string(array_agg(lineitem_enc.l_discount_DET), ',') from lineitem_enc, part_enc where ((((((((((part_enc.p_partkey_DET) = (lineitem_enc.l_partkey_DET)) and ((part_enc.p_brand_DET) = (:0))) and (part_enc.p_container_DET in ( :1, :2, :3, :4 ))) and ((lineitem_enc.l_quantity_OPE) >= (:5))) and ((lineitem_enc.l_quantity_OPE) <= (:6))) and (((part_enc.p_size_OPE) >= (:7)) and ((part_enc.p_size_OPE) <= (:8)))) and (lineitem_enc.l_shipmode_DET in ( :9, :10 ))) and ((lineitem_enc.l_shipinstruct_DET) = (:11))) or (((((((((part_enc.p_partkey_DET) = (lineitem_enc.l_partkey_DET)) and ((part_enc.p_brand_DET) = (:12))) and (part_enc.p_container_DET in ( :13, :14, :15, :16 ))) and ((lineitem_enc.l_quantity_OPE) >= (:17))) and ((lineitem_enc.l_quantity_OPE) <= (:18))) and (((part_enc.p_size_OPE) >= (:19)) and ((part_enc.p_size_OPE) <= (:20)))) and (lineitem_enc.l_shipmode_DET in ( :21, :22 ))) and ((lineitem_enc.l_shipinstruct_DET) = (:23)))) or (((((((((part_enc.p_partkey_DET) = (lineitem_enc.l_partkey_DET)) and ((part_enc.p_brand_DET) = (:24))) and (part_enc.p_container_DET in ( :25, :26, :27, :28 ))) and ((lineitem_enc.l_quantity_OPE) >= (:29))) and ((lineitem_enc.l_quantity_OPE) <= (:30))) and (((part_enc.p_size_OPE) >= (:31)) and ((part_enc.p_size_OPE) <= (:32)))) and (lineitem_enc.l_shipmode_DET in ( :33, :34 ))) and ((lineitem_enc.l_shipinstruct_DET) = (:35)))", {db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 5, true), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 6, true)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({}))));
  op->open(ctx);
  while (op->has_more(ctx)) {
    physical_operator::db_tuple_vec v;
    op->next(ctx, v);
