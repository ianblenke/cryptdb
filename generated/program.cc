...
static void query_12(exec_context& ctx) {
  physical_operator* op = new local_transform_op({local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DOUBLE, 8, oNONE, SECLEVEL::PLAIN, 0, false), new div_node(new mult_node(new double_literal_node(100.000000), new function_call_node("hom_get_pos", {new tuple_pos_node(1), new int_literal_node(0)})), new function_call_node("hom_get_pos", {new tuple_pos_node(0), new int_literal_node(0)})))), }, new local_decrypt_op({0, 1}, new remote_sql_op(new param_generator_id13, "select agg_hash(:0, '/space/stephentu/data/" DB_DATABASE "/lineitem_enc/row_pack/disc_price', 256, 12, true, " LINEITEM_ENC_NAME ".row_id), agg_hash(:1, '/space/stephentu/data/" DB_DATABASE "/lineitem_enc/row_pack/disc_price', 256, 12, true, case when searchSWP(:2, :3, " PART_ENC_NAME ".p_type_SWP) then " LINEITEM_ENC_NAME ".row_id else NULL end) from " LINEITEM_ENC_NAME ", " PART_ENC_NAME " where (((" LINEITEM_ENC_NAME ".l_partkey_DET) = (" PART_ENC_NAME ".p_partkey_DET)) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) >= (:4))) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) < (:5))", {db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false), db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({}))));
  op->open(ctx);
  while (op->has_more(ctx)) {
    physical_operator::db_tuple_vec v;
    op->next(ctx, v);
