...
virtual param_map get_param_map(exec_context& ctx) {
  param_map m;
  m[0] = db_elem((int64_t)encrypt_date_ope(ctx.crypto, 1023009 /*1998-1-1*/, 10, true));
  return m;
}
};
...
virtual param_map get_param_map(exec_context& ctx) {
  ...
  m[2] = db_elem((int64_t)encrypt_decimal_15_2_det(ctx.crypto, 0 /*0*/, 0, false));
  m[3] = db_elem((int64_t)encrypt_date_ope(ctx.crypto, 1022177 /*1996-7-1*/, 10, true));
  m[4] = db_elem((int64_t)encrypt_date_ope(ctx.crypto, 1022209 /*1996-8-1*/, 10, true));
  return m;
}
};
...
static void query_0(exec_context& ctx) {
  physical_operator* op = new local_order_by(
    ...
    new local_transform_op(
      {local_transform_op::trfm_desc(0), local_transform_op::trfm_desc(1), local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oNONE, SECLEVEL::PLAIN, 0, false), new sum_node(new tuple_pos_node(2), false))), local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oNONE, SECLEVEL::PLAIN, 0, false), new sum_node(new tuple_pos_node(3), false))), local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oNONE, SECLEVEL::PLAIN, 0, false), new sum_node(new mult_node(new tuple_pos_node(3), new sub_node(new int_literal_node(1), new tuple_pos_node(4))), false))), local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oNONE, SECLEVEL::PLAIN, 0, false), new sum_node(new mult_node(new mult_node(new tuple_pos_node(3), new sub_node(new int_literal_node(1), new tuple_pos_node(4))), new add_node(new int_literal_node(1), new tuple_pos_node(5))), false))), local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DOUBLE, 8, oNONE, SECLEVEL::PLAIN, 0, false), new avg_node(new tuple_pos_node(2), false))), local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DOUBLE, 8, oNONE, SECLEVEL::PLAIN, 0, false), new avg_node(new tuple_pos_node(3), false))), local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DOUBLE, 8, oNONE, SECLEVEL::PLAIN, 0, false), new avg_node(new tuple_pos_node(4), false))), local_transform_op::trfm_desc(6), },
      new local_decrypt_op(
        {0, 1, 2, 3, 4, 5},
        new remote_sql_op(new param_generator_id0, "select " LINEITEM_ENC_NAME ".l_returnflag_DET, " LINEITEM_ENC_NAME ".l_linestatus_DET, group_serializer(" LINEITEM_ENC_NAME ".l_quantity_DET), group_serializer(" LINEITEM_ENC_NAME ".l_extendedprice_DET), group_serializer(" LINEITEM_ENC_NAME ".l_discount_DET), group_serializer(" LINEITEM_ENC_NAME ".l_tax_DET), count(*) as count_order from " LINEITEM_ENC_NAME " where (" LINEITEM_ENC_NAME ".l_shipdate_OPE) <= (:0) group by " LINEITEM_ENC_NAME ".l_returnflag_DET, " LINEITEM_ENC_NAME ".l_linestatus_DET", {db_column_desc(db_elem::TYPE_CHAR, 1, oDET, SECLEVEL::DET, 8, false), db_column_desc(db_elem::TYPE_CHAR, 1, oDET, SECLEVEL::DET, 9, false), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 4, true), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 5, true), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 6, true), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 7, true), db_column_desc(db_elem::TYPE_INT, 4, oNONE, SECLEVEL::PLAIN, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
    )
  )
  ;
  op->open(ctx);
...
static void query_5(exec_context& ctx) {
  // the optimizer was suggesting an pre-computed value for
  // (l_extendedprice * l_discount), but since we don't have computed,
  // we just project it. This makes our perf *worse* than what the optimizer is
  // telling us, but it should be good enough to just make the point.
  physical_operator* op = new local_transform_op(
    ...
    new local_decrypt_op(
      {0, 1},
      new remote_sql_op(new param_generator_id5, "select group_serializer(" LINEITEM_ENC_NAME ".l_extendedprice_DET), group_serializer(l_discount_DET) from " LINEITEM_ENC_NAME " where (((((" LINEITEM_ENC_NAME ".l_shipdate_OPE) >= (:0)) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) < (:1))) and ((" LINEITEM_ENC_NAME ".l_discount_OPE) >= (:2))) and ((" LINEITEM_ENC_NAME ".l_discount_OPE) <= (:3))) and ((" LINEITEM_ENC_NAME ".l_quantity_OPE) < (:4))", {db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 5, true), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 6, true)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
  )
  ;
  op->open(ctx);
  while (op->has_more(ctx)) {
...
static void query_12(exec_context& ctx) {
  physical_operator* op = new local_transform_op(
    {local_transform_op::trfm_desc(std::make_pair(db_column_desc(db_elem::TYPE_DOUBLE, 8, oNONE, SECLEVEL::PLAIN, 0, false), new div_node(new mult_node(new double_literal_node(100.000000), new sum_node(new tuple_pos_node(0), false)), new sum_node(new tuple_pos_node(1), false)))), },
    new local_decrypt_op(
      {0, 1},
      new remote_sql_op(new param_generator_id13, "select group_serializer(case when searchSWP(:0, :1, " PART_ENC_NAME ".p_type_SWP) then " LINEITEM_ENC_NAME ".l_disc_price_DET else :2 end), group_serializer(" LINEITEM_ENC_NAME ".l_disc_price_DET) from " LINEITEM_ENC_NAME ", " PART_ENC_NAME " where (((" LINEITEM_ENC_NAME ".l_partkey_DET) = (" PART_ENC_NAME ".p_partkey_DET)) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) >= (:3))) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) < (:4))", {db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 0, true), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 0, true)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
  )
  ;
  op->open(ctx);
  while (op->has_more(ctx)) {
  ...
  physical_operator* op = new local_limit(100,
    new local_order_by(
      ...
      new local_transform_op(
        ...
        new local_decrypt_op(
          {0, 1, 2, 3, 4, 5},
          new remote_sql_op(new param_generator_id17, "select " CUSTOMER_ENC_NAME ".c_name_DET, " CUSTOMER_ENC_NAME ".c_custkey_DET, " ORDERS_ENC_NAME ".o_orderkey_DET, " ORDERS_ENC_NAME ".o_orderdate_DET, " ORDERS_ENC_NAME ".o_totalprice_DET, group_serializer(" LINEITEM_ENC_NAME ".l_quantity_DET) from " CUSTOMER_ENC_NAME ", " ORDERS_ENC_NAME ", " LINEITEM_ENC_NAME " where ((" ORDERS_ENC_NAME ".o_orderkey_DET in ( :_subselect$0 )) and ((" CUSTOMER_ENC_NAME ".c_custkey_DET) = (" ORDERS_ENC_NAME ".o_custkey_DET))) and ((" ORDERS_ENC_NAME ".o_orderkey_DET) = (" LINEITEM_ENC_NAME ".l_orderkey_DET)) group by " CUSTOMER_ENC_NAME ".c_name_DET, " CUSTOMER_ENC_NAME ".c_custkey_DET, " ORDERS_ENC_NAME ".o_orderkey_DET, " ORDERS_ENC_NAME ".o_orderdate_DET, " ORDERS_ENC_NAME ".o_totalprice_DET", {db_column_desc(db_elem::TYPE_STRING, 25, oDET, SECLEVEL::DET, 1, false), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, false), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, false), db_column_desc(db_elem::TYPE_DATE, 3, oDET, SECLEVEL::DET, 4, false), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 3, false), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 4, true)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({std::pair<std::string, physical_operator*>("_subselect$0", new local_transform_op(
            ...
            new local_group_filter(
              ...
              new local_decrypt_op(
                {0},
                new remote_sql_op(new param_generator_id16, "select group_serializer(" LINEITEM_ENC_NAME ".l_quantity_DET), " LINEITEM_ENC_NAME ".l_orderkey_DET from " LINEITEM_ENC_NAME " group by " LINEITEM_ENC_NAME ".l_orderkey_DET having (count(*)) >= (7)", {db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 4, true), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
              , {
              }
            )
          )
