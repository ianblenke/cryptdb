...
static void query_4(exec_context& ctx) {
  physical_operator* op = new local_order_by(
    ...
    new local_transform_op(
      ...
      new local_decrypt_op(
        {0, 1},
        new remote_sql_op(new param_generator_id4, "select " NATION_ENC_NAME ".n_name_DET, agg_hash(:0, '/space/stephentu/data/" DB_DATABASE "/lineitem_enc/row_pack/disc_price', 256, 12, true, " LINEITEM_ENC_NAME ".row_id) from " CUSTOMER_ENC_NAME ", " ORDERS_ENC_NAME ", " LINEITEM_ENC_NAME ", " SUPPLIER_ENC_NAME ", " NATION_ENC_NAME ", " REGION_ENC_NAME " where (((((((((" CUSTOMER_ENC_NAME ".c_custkey_DET) = (" ORDERS_ENC_NAME ".o_custkey_DET)) and ((" LINEITEM_ENC_NAME ".l_orderkey_DET) = (" ORDERS_ENC_NAME ".o_orderkey_DET))) and ((" LINEITEM_ENC_NAME ".l_suppkey_DET) = (" SUPPLIER_ENC_NAME ".s_suppkey_DET))) and ((" CUSTOMER_ENC_NAME ".c_nationkey_DET) = (" SUPPLIER_ENC_NAME ".s_nationkey_DET))) and ((" SUPPLIER_ENC_NAME ".s_nationkey_DET) = (" NATION_ENC_NAME ".n_nationkey_DET))) and ((" NATION_ENC_NAME ".n_regionkey_DET) = (" REGION_ENC_NAME ".r_regionkey_DET))) and ((" REGION_ENC_NAME ".r_name_DET) = (:1))) and ((" ORDERS_ENC_NAME ".o_orderdate_OPE) >= (:2))) and ((" ORDERS_ENC_NAME ".o_orderdate_OPE) < (:3)) group by " NATION_ENC_NAME ".n_name_DET", {db_column_desc(db_elem::TYPE_STRING, 25, oDET, SECLEVEL::DET, 1, false), db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
    )
  )
  ;
  op->open(ctx);
...
static void query_5(exec_context& ctx) {
  physical_operator* op = new local_transform_op(
    ...
    new local_decrypt_op(
      {0},
      new remote_sql_op(new param_generator_id5, "select agg_hash(:0, '/space/stephentu/data/" DB_DATABASE "/lineitem_enc/row_pack/revenue', 256, 12, true, " LINEITEM_ENC_NAME ".row_id) from " LINEITEM_ENC_NAME " where (((((" LINEITEM_ENC_NAME ".l_shipdate_OPE) >= (:1)) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) < (:2))) and ((" LINEITEM_ENC_NAME ".l_discount_OPE) >= (:3))) and ((" LINEITEM_ENC_NAME ".l_discount_OPE) <= (:4))) and ((" LINEITEM_ENC_NAME ".l_quantity_OPE) < (:5))", {db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
  )
  ;
  op->open(ctx);
  while (op->has_more(ctx)) {
...
static void query_12(exec_context& ctx) {
  physical_operator* op = new local_transform_op(
    ...
    new local_decrypt_op(
      {0, 1},
      new remote_sql_op(new param_generator_id13, "select agg_hash(:0, '/space/stephentu/data/" DB_DATABASE "/lineitem_enc/row_pack/disc_price', 256, 12, true, case when searchSWP(:1, :2, " PART_ENC_NAME ".p_type_SWP) then " LINEITEM_ENC_NAME ".row_id else NULL end), agg_hash(:3, '/space/stephentu/data/" DB_DATABASE "/lineitem_enc/row_pack/disc_price', 256, 12, true, " LINEITEM_ENC_NAME ".row_id) from " LINEITEM_ENC_NAME ", " PART_ENC_NAME " where (((" LINEITEM_ENC_NAME ".l_partkey_DET) = (" PART_ENC_NAME ".p_partkey_DET)) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) >= (:4))) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) < (:5))", {db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false), db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
  )
  ;
  op->open(ctx);
  while (op->has_more(ctx)) {
  ...
  physical_operator* op = new local_transform_op(
    ...
    new local_decrypt_op(
      ...
      new remote_sql_op(new param_generator_id19, "select agg_ident(custsale.cntrycode_DET), count(*) as numcust, agg_hash(:0, '/space/stephentu/data/" DB_DATABASE "/customer_enc/row_pack/acctbal', 256, 12, true, custsale.abal) from ( select " CUSTOMER_ENC_NAME ".c_phone_prefix_OPE as cntrycode, " CUSTOMER_ENC_NAME ".c_phone_prefix_DET as cntrycode_DET, " CUSTOMER_ENC_NAME ".row_id as abal from " CUSTOMER_ENC_NAME " where ((" CUSTOMER_ENC_NAME ".c_phone_prefix_DET in ( :1, :2, :3, :4, :5, :6, :7 )) and ((" CUSTOMER_ENC_NAME ".c_acctbal_OPE) > (:_subselect$154))) and (not ( exists ( (select 1 from " ORDERS_ENC_NAME " where (" ORDERS_ENC_NAME ".o_custkey_DET) = (" CUSTOMER_ENC_NAME ".c_custkey_DET)) ) )) ) as custsale group by custsale.cntrycode order by custsale.cntrycode ASC", {db_column_desc(db_elem::TYPE_STRING, 2, oDET, SECLEVEL::DET, 4, false), db_column_desc(db_elem::TYPE_INT, 4, oNONE, SECLEVEL::PLAIN, 0, false), db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({std::pair<std::string, physical_operator*>("_subselect$154", new local_encrypt_op({std::pair<size_t, db_column_desc>(0, db_column_desc(db_elem::TYPE_DECIMAL_15_2, 8, oOPE, SECLEVEL::OPE, 5, false))}, new local_transform_op(
        ...
        new local_decrypt_op(
          {0},
          new remote_sql_op(new param_generator_id18, "select agg_hash(:0, '/space/stephentu/data/" DB_DATABASE "/customer_enc/row_pack/acctbal', 256, 12, true, " CUSTOMER_ENC_NAME ".row_id), count(*) from " CUSTOMER_ENC_NAME " where ((" CUSTOMER_ENC_NAME ".c_acctbal_OPE) > (:1)) and (" CUSTOMER_ENC_NAME ".c_phone_prefix_DET in ( :2, :3, :4, :5, :6, :7, :8 ))", {db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false), db_column_desc(db_elem::TYPE_INT, 4, oNONE, SECLEVEL::PLAIN, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
      )
      ))})))
  )
  ;
