    ...
    new local_decrypt_op(
      ...
      new remote_sql_op(new param_generator_id16, "select " CUSTOMER_ENC_NAME ".c_name_DET, " CUSTOMER_ENC_NAME ".c_custkey_DET, " ORDERS_ENC_NAME ".o_orderkey_DET, agg_ident(" ORDERS_ENC_NAME ".o_orderdate_DET), agg_ident(" ORDERS_ENC_NAME ".o_totalprice_DET), agg_hash(:0, '/space/stephentu/data/" DB_DATABASE "/lineitem_enc/row_pack/quantity', 256, 12, false, " LINEITEM_ENC_NAME ".row_id) from " CUSTOMER_ENC_NAME ", " ORDERS_ENC_NAME ", " LINEITEM_ENC_NAME " where ((" ORDERS_ENC_NAME ".o_orderkey_DET in ( :_subselect$2 )) and ((" CUSTOMER_ENC_NAME ".c_custkey_DET) = (" ORDERS_ENC_NAME ".o_custkey_DET))) and ((" ORDERS_ENC_NAME ".o_orderkey_DET) = (" LINEITEM_ENC_NAME ".l_orderkey_DET)) group by " CUSTOMER_ENC_NAME ".c_name_DET, " CUSTOMER_ENC_NAME ".c_custkey_DET, " ORDERS_ENC_NAME ".o_orderkey_DET, " ORDERS_ENC_NAME ".o_orderdate_OPE, " ORDERS_ENC_NAME ".o_totalprice_OPE order by " ORDERS_ENC_NAME ".o_totalprice_OPE DESC, " ORDERS_ENC_NAME ".o_orderdate_OPE ASC limit 100", {db_column_desc(db_elem::TYPE_STRING, 25, oDET, SECLEVEL::DET, 1, false), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, false), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, false), db_column_desc(db_elem::TYPE_DATE, 3, oDET, SECLEVEL::DET, 4, false), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 3, false), db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({std::pair<std::string, physical_operator*>("_subselect$2", new local_transform_op(
        ...
        new local_group_filter(
          ...
          new local_decrypt_op(
            {0},
            new remote_sql_op(new param_generator_id15, "select agg_hash(:0, '/space/stephentu/data/" DB_DATABASE "/lineitem_enc/row_pack/quantity', 256, 12, false, " LINEITEM_ENC_NAME ".row_id), " LINEITEM_ENC_NAME ".l_orderkey_DET from " LINEITEM_ENC_NAME " group by " LINEITEM_ENC_NAME ".l_orderkey_DET having (count(*)) >= (7)", {db_column_desc(db_elem::TYPE_STRING, 2147483647, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
          , {
          }
        )
      )
        ...
        new local_transform_op(
          ...
          new local_filter_op(
            ...
            new local_transform_op(
              ...
              new local_decrypt_op(
                {1, 2},
                new remote_sql_op(new param_generator_id20, "select " PARTSUPP_ENC_NAME ".ps_suppkey_DET, agg_ident(" PARTSUPP_ENC_NAME ".ps_availqty_DET), agg_hash(:0, '/space/stephentu/data/" DB_DATABASE "/lineitem_enc/row_pack/quantity', 256, 12, false, " LINEITEM_ENC_NAME ".row_id) from " PARTSUPP_ENC_NAME ", " LINEITEM_ENC_NAME " where (((((" LINEITEM_ENC_NAME ".l_partkey_DET) = (" PARTSUPP_ENC_NAME ".ps_partkey_DET)) and ((" LINEITEM_ENC_NAME ".l_suppkey_DET) = (" PARTSUPP_ENC_NAME ".ps_suppkey_DET))) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) >= (:1))) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) < (:2))) and (" PARTSUPP_ENC_NAME ".ps_partkey_DET in ( select " PART_ENC_NAME ".p_partkey_DET from " PART_ENC_NAME " where searchSWP(:3, :4, " PART_ENC_NAME ".p_name_SWP) )) group by " PARTSUPP_ENC_NAME ".ps_partkey_DET, " PARTSUPP_ENC_NAME ".ps_suppkey_DET", {db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DETJOIN, 1, false), db_column_desc(db_elem::TYPE_INT, 4, oDET, SECLEVEL::DET, 2, false), db_column_desc(db_elem::TYPE_STRING, 15, oAGG, SECLEVEL::SEMANTIC_AGG, 0, false)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
            )
            ,{
            }
          )
