...
virtual param_map get_param_map(exec_context& ctx) {
  param_map m;
  m[0] = db_elem((int64_t)encrypt_date_ope(ctx.crypto, 1023009 /*1998-1-1*/, _FP(10), _FJ(false)));
  return m;
}
};
...
virtual param_map get_param_map(exec_context& ctx) {
  ...
  m[1] = db_elem((int64_t)encrypt_date_ope(ctx.crypto, 1021548 /*1995-3-12*/, _FP(10), _FJ(false)));
  return m;
}
};
