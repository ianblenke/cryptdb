...
static void query_0(ConnectNew& conn) {
  DBResultNew* dbres;
  conn.execute("select l_returnflag, l_linestatus, sum(l_quantity) as sum_qty, sum(l_extendedprice) as sum_base_price, sum((l_extendedprice) * ((100) - (l_discount))) as sum_disc_price, sum(((l_extendedprice) * ((100) - (l_discount))) * ((100) + (l_tax))) as sum_charge, avg(l_quantity) as avg_qty, avg(l_extendedprice) as avg_price, avg(l_discount) as avg_disc, count(*) as count_order from " LINEITEM_NAME " where (l_shipdate) <= (date '1998-01-01') group by l_returnflag, l_linestatus order by l_returnflag ASC, l_linestatus ASC", dbres);
...
static void query_5(ConnectNew& conn) {
  DBResultNew* dbres;
  conn.execute("select sum((l_extendedprice) * (l_discount)) as revenue from " LINEITEM_NAME " where ((((l_shipdate) >= (date '1993-01-01')) and ((l_shipdate) < ((date '1993-01-01') + (interval '1' YEAR)))) and (((l_discount) >= ((5) - (1))) and ((l_discount) <= ((5) + (1))))) and ((l_quantity) < (2500))", dbres);
  ResType res = dbres->unpack();
  assert(res.ok);
  for (auto &row : res.rows) {
    std::cout << row[0].data;
...
static void query_10(ConnectNew& conn) {
  DBResultNew* dbres;
  conn.execute("select ps_partkey, sum((ps_supplycost) * (ps_availqty)) as value from " PARTSUPP_NAME ", " SUPPLIER_NAME ", " NATION_NAME " where (((ps_suppkey) = (s_suppkey)) and ((s_nationkey) = (n_nationkey))) and ((n_name) = ('ARGENTINA')) group by ps_partkey having (sum((ps_supplycost) * (ps_availqty))) > ((select (sum((ps_supplycost) * (ps_availqty))) * (1.0E-4) from " PARTSUPP_NAME ", " SUPPLIER_NAME ", " NATION_NAME " where (((ps_suppkey) = (s_suppkey)) and ((s_nationkey) = (n_nationkey))) and ((n_name) = ('ARGENTINA')))) order by value DESC", dbres);
  ResType res = dbres->unpack();
  assert(res.ok);
  for (auto &row : res.rows) {
    std::cout << row[0].data;
...
static void query_12(ConnectNew& conn) {
  DBResultNew* dbres;
  conn.execute("select ((100.0) * (sum(case when (p_type) like ('PROMO%') then (l_extendedprice) * ((100) - (l_discount)) else 0 end))) / (sum((l_extendedprice) * ((100) - (l_discount)))) as promo_revenue from " LINEITEM_NAME ", " PART_NAME " where (((l_partkey) = (p_partkey)) and ((l_shipdate) >= (date '1996-07-01'))) and ((l_shipdate) < ((date '1996-07-01') + (interval '1' MONTH)))", dbres);
  ResType res = dbres->unpack();
  assert(res.ok);
  for (auto &row : res.rows) {
    std::cout << row[0].data;
...
static void query_16(ConnectNew& conn) {
  DBResultNew* dbres;
  conn.execute("select cntrycode, count(*) as numcust, sum(abs(abal)) as totacctbal from ( select substring(c_phone from 1 for 2) as cntrycode, c_acctbal as abal from " CUSTOMER_NAME " where ((substring(c_phone from 1 for 2) in ( '41', '26', '36', '27', '38', '37', '22' )) and ((c_acctbal) > ((select avg(c_acctbal) from " CUSTOMER_NAME " where (substring(c_phone from 1 for 2) in ( '41', '26', '36', '27', '38', '37', '22' )))))) and (not ( exists ( (select * from " ORDERS_NAME " where (o_custkey) = (c_custkey)) ) )) ) as custsale group by cntrycode order by cntrycode ASC", dbres);
  ResType res = dbres->unpack();
  assert(res.ok);
  for (auto &row : res.rows) {
    std::cout << row[0].data;
