...
static int
encode_yyyy_mm_dd(const std::string &datestr) {
  int year, month, day;
...
static void query_0(ConnectNew& conn) {
  ...
  buf << "select l_returnflag, l_linestatus, fast_sum(l_quantity) as sum_qty, fast_sum(l_extendedprice) as sum_base_price, fast_sum((l_extendedprice) * ((100) - (l_discount))) as sum_disc_price, fast_sum(((l_extendedprice) * ((100) - (l_discount))) * ((100) + (l_tax))) as sum_charge, fast_sum(l_quantity) / count(*) as avg_qty, fast_sum(l_extendedprice) / count(*) as avg_price, fast_sum(l_discount) / count(*) as avg_disc, count(*) as count_order from " LINEITEM_NAME " where (l_shipdate) <= ("
      << encode_yyyy_mm_dd("1998-01-01")
      << ") group by l_returnflag, l_linestatus order by l_returnflag ASC, l_linestatus ASC";
...
static void query_2(ConnectNew& conn) {
  DBResultNew* dbres;
  std::ostringstream buf;
  buf << "select l_orderkey, fast_sum((l_extendedprice) * ((100) - (l_discount))) as revenue, o_orderdate, o_shippriority from " CUSTOMER_NAME ", " ORDERS_NAME ", " LINEITEM_NAME " where (((((c_mktsegment) = ('FURNITURE')) and ((c_custkey) = (o_custkey))) and ((l_orderkey) = (o_orderkey))) and ((o_orderdate) < ("
      << encode_yyyy_mm_dd("1995-03-12") /*date '1995-03-12'*/
      << "))) and ((l_shipdate) > ("
      << encode_yyyy_mm_dd("1995-03-12") /*date '1995-03-12'*/
      << ")) group by l_orderkey, o_orderdate, o_shippriority order by revenue DESC, o_orderdate ASC limit 10";
  conn.execute(buf.str(), dbres);
  ResType res = dbres->unpack();
  assert(res.ok);
  for (auto &row : res.rows) {
...
static void query_3(ConnectNew& conn) {
  DBResultNew* dbres;
  const char * p = "select o_orderpriority, count(*) as order_count from " ORDERS_NAME " where (((o_orderdate) >= 1021281) and ((o_orderdate) < 1021505)) and (exists ( (select * from " LINEITEM_NAME " where ((l_orderkey) = (o_orderkey)) and ((l_commitdate) < (l_receiptdate))) )) group by o_orderpriority order by o_orderpriority ASC";
  std::cerr << p << std::endl;
...
static void query_4(ConnectNew& conn) {
  DBResultNew* dbres;
  std::ostringstream buf;
  buf << "select n_name, fast_sum((l_extendedprice) * ((100) - (l_discount))) as revenue from " CUSTOMER_NAME ", " ORDERS_NAME ", " LINEITEM_NAME ", " SUPPLIER_NAME ", " NATION_NAME ", " REGION_NAME " where (((((((((c_custkey) = (o_custkey)) and ((l_orderkey) = (o_orderkey))) and ((l_suppkey) = (s_suppkey))) and ((c_nationkey) = (s_nationkey))) and ((s_nationkey) = (n_nationkey))) and ((n_regionkey) = (r_regionkey))) and ((r_name) = ('MIDDLE EAST'))) and ((o_orderdate) >= ("
      << encode_yyyy_mm_dd("1993-01-01") /*date '1993-01-01'*/
      << "))) and ((o_orderdate) < ("
      << encode_yyyy_mm_dd("1994-01-01") /*(date '1993-01-01') + (interval '1' YEAR)*/
...
static void query_5(ConnectNew& conn) {
  DBResultNew* dbres;
  std::ostringstream buf;
  buf << "select fast_sum((l_extendedprice) * (l_discount)) as revenue from " LINEITEM_NAME " where ((((l_shipdate) >= ("
      << encode_yyyy_mm_dd("1993-01-01") /*date '1993-01-01'*/
      << ")) and ((l_shipdate) < ("
      << encode_yyyy_mm_dd("1994-01-01") /*(date '1993-01-01') + (interval '1' YEAR)*/
      << "))) and (((l_discount) >= ((5) - (1))) and ((l_discount) <= ((5) + (1))))) and ((l_quantity) < (2500))";
  conn.execute(buf.str(), dbres);
  ResType res = dbres->unpack();
  assert(res.ok);
  for (auto &row : res.rows) {
...
static void query_6(ConnectNew& conn) {
  DBResultNew* dbres;
  std::ostringstream buf;
  buf << "select supp_nation, cust_nation, l_year, fast_sum(volume) as revenue from ( select n1.n_name as supp_nation, n2.n_name as cust_nation, (l_shipdate >> 9) as l_year, (l_extendedprice) * ((100) - (l_discount)) as volume from " SUPPLIER_NAME ", " LINEITEM_NAME ", " ORDERS_NAME ", " CUSTOMER_NAME ", " NATION_NAME " n1, " NATION_NAME " n2 where (((((((s_suppkey) = (l_suppkey)) and ((o_orderkey) = (l_orderkey))) and ((c_custkey) = (o_custkey))) and ((s_nationkey) = (n1.n_nationkey))) and ((c_nationkey) = (n2.n_nationkey))) and ((((n1.n_name) = ('SAUDI ARABIA')) and ((n2.n_name) = ('ARGENTINA'))) or (((n1.n_name) = ('ARGENTINA')) and ((n2.n_name) = ('SAUDI ARABIA'))))) and (((l_shipdate) >= ("
      << encode_yyyy_mm_dd("1995-01-01") /*date '1995-01-01'*/
      << ")) and ((l_shipdate) <= ("
      << encode_yyyy_mm_dd("1996-12-31") /*date '1996-12-31'*/
      << "))) ) as shipping group by supp_nation, cust_nation, l_year order by supp_nation ASC, cust_nation ASC, l_year ASC";
  conn.execute(buf.str(), dbres);
  ResType res = dbres->unpack();
  assert(res.ok);
  for (auto &row : res.rows) {
...
static void query_7(ConnectNew& conn) {
  DBResultNew* dbres;
  std::ostringstream buf;
  buf << "select o_year, (fast_sum(case when (nation) = ('ARGENTINA') then volume else 0 end)) / (fast_sum(volume)) as mkt_share from ( select (o_orderdate >> 9) as o_year, (l_extendedprice) * ((100) - (l_discount)) as volume, n2.n_name as nation from " PART_NAME ", " SUPPLIER_NAME ", " LINEITEM_NAME ", " ORDERS_NAME ", " CUSTOMER_NAME ", " NATION_NAME " n1, " NATION_NAME " n2, " REGION_NAME " where ((((((((((p_partkey) = (l_partkey)) and ((s_suppkey) = (l_suppkey))) and ((l_orderkey) = (o_orderkey))) and ((o_custkey) = (c_custkey))) and ((c_nationkey) = (n1.n_nationkey))) and ((n1.n_regionkey) = (r_regionkey))) and ((r_name) = ('AMERICA'))) and ((s_nationkey) = (n2.n_nationkey))) and (((o_orderdate) >= ("
      << encode_yyyy_mm_dd("1995-01-01") /*date '1995-01-01'*/
      << ")) and ((o_orderdate) <= ("
      << encode_yyyy_mm_dd("1996-12-31") /*date '1996-12-31'*/
      << ")))) and ((p_type) = ('PROMO ANODIZED BRASS')) ) as all_nations group by o_year order by o_year ASC";
  conn.execute(buf.str(), dbres);
  ResType res = dbres->unpack();
  assert(res.ok);
  for (auto &row : res.rows) {
...
static void query_9(ConnectNew& conn) {
  DBResultNew* dbres;
  std::ostringstream buf;
  buf << "select c_custkey, c_name, fast_sum((l_extendedprice) * ((100) - (l_discount))) as revenue, c_acctbal, n_name, c_address, c_phone, c_comment from " CUSTOMER_NAME ", " ORDERS_NAME ", " LINEITEM_NAME ", " NATION_NAME " where ((((((c_custkey) = (o_custkey)) and ((l_orderkey) = (o_orderkey))) and ((o_orderdate) >= ("
      << encode_yyyy_mm_dd("1994-08-01") /*date '1994-08-01'*/
      << "))) and ((o_orderdate) < ("
      << encode_yyyy_mm_dd("1994-11-01") /*(date '1994-08-01') + (interval '3' MONTH)*/
      << "))) and ((l_returnflag) = ('R'))) and ((c_nationkey) = (n_nationkey)) group by c_custkey, c_name, c_acctbal, c_phone, n_name, c_address, c_comment order by revenue DESC limit 20";
  conn.execute(buf.str(), dbres);
  ResType res = dbres->unpack();
  assert(res.ok);
  for (auto &row : res.rows) {
...
static void query_11(ConnectNew& conn) {
  DBResultNew* dbres;
  std::ostringstream buf;
  buf << "select l_shipmode, fast_sum(case when ((o_orderpriority) = ('1-URGENT')) or ((o_orderpriority) = ('2-HIGH')) then 1 else 0 end) as high_line_count, fast_sum(case when ((o_orderpriority) <> ('1-URGENT')) and ((o_orderpriority) <> ('2-HIGH')) then 1 else 0 end) as low_line_count from " ORDERS_NAME ", " LINEITEM_NAME " where ((((((o_orderkey) = (l_orderkey)) and (l_shipmode in ( 'AIR', 'FOB' ))) and ((l_commitdate) < (l_receiptdate))) and ((l_shipdate) < (l_commitdate))) and ((l_receiptdate) >= ("
      << encode_yyyy_mm_dd("1993-01-01") /*date '1993-01-01'*/
      << "))) and ((l_receiptdate) < ("
      << encode_yyyy_mm_dd("1994-01-01") /*(date '1993-01-01') + (interval '1' YEAR)*/
      << ")) group by l_shipmode order by l_shipmode ASC";
  conn.execute(buf.str(), dbres);
  ResType res = dbres->unpack();
  assert(res.ok);
  for (auto &row : res.rows) {
...
static void query_12(ConnectNew& conn) {
  DBResultNew* dbres;
  std::ostringstream buf;
  buf << "select ((100.0) * (fast_sum(case when (p_type) like ('PROMO%') then (l_extendedprice) * ((100) - (l_discount)) else 0 end))) / (fast_sum((l_extendedprice) * ((100) - (l_discount)))) as promo_revenue from " LINEITEM_NAME ", " PART_NAME " where (((l_partkey) = (p_partkey)) and ((l_shipdate) >= ("
      << encode_yyyy_mm_dd("1996-07-01") /*date '1996-07-01'*/
      << "))) and ((l_shipdate) < ("
      << encode_yyyy_mm_dd("1996-08-01") /*(date '1996-07-01') + (interval '1' MONTH)*/
      << "))";
  conn.execute(buf.str(), dbres);
  ResType res = dbres->unpack();
  assert(res.ok);
...
static void query_17(ConnectNew& conn) {
  DBResultNew* dbres;
  std::ostringstream buf;
  buf << "select s_name, s_address from " SUPPLIER_NAME ", " NATION_NAME " where s_suppkey in ( select ps_suppkey from ( select ps_suppkey, fast_sum(ps_availqty) * 100 as x, fast_sum(l_quantity) as y from " PARTSUPP_NAME ", " LINEITEM_NAME " where l_partkey = ps_partkey and l_suppkey = ps_suppkey and l_shipdate >= "
      << encode_yyyy_mm_dd("1997-01-01") /*date '1997-01-01'*/
      << "and l_shipdate < "
      << encode_yyyy_mm_dd("1998-01-01") /*date '1997-01-01' + interval '1' year*/
      << "and ps_partkey in ( select p_partkey from " PART_NAME " where p_name like '%khaki%') group by ps_partkey, ps_suppkey) as anon1 where x > 0.5 * y) and s_nationkey = n_nationkey and n_name = 'ALGERIA' order by s_name";
  conn.execute(buf.str(), dbres);
  ResType res = dbres->unpack();
  assert(res.ok);
  for (auto &row : res.rows) {
