...
virtual param_map get_param_map(exec_context& ctx) {
  ...
  m[2] = db_elem(str_reverse(str_resize(encrypt_decimal_15_2_ope(ctx.crypto, 2500 /*25*/, 4, false), 16)));
  return m;
}
};
  ...
  physical_operator* op = new local_transform_op(
    ...
    new local_decrypt_op(
      ...
      new local_filter_op(
        ...
        new local_decrypt_op(
          {0},
          new remote_sql_op(new param_generator_id5, "select group_serializer(" LINEITEM_ENC_NAME ".l_discount_DET), group_serializer(" LINEITEM_ENC_NAME ".l_extendedprice_DET) from " LINEITEM_ENC_NAME " where (((" LINEITEM_ENC_NAME ".l_shipdate_OPE) >= (:0)) and ((" LINEITEM_ENC_NAME ".l_shipdate_OPE) < (:1))) and ((" LINEITEM_ENC_NAME ".l_quantity_OPE) < (:2))", {db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 6, true), db_column_desc(db_elem::TYPE_DECIMAL_15_2, 15, oDET, SECLEVEL::DET, 5, true)}, {}, util::map_from_pair_vec<std::string, physical_operator*>({})))
        ,{
        }
      )
    )
